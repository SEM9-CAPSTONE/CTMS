...
void main() {
  ...
  testWidgets('registers successfully with a valid email and reaches the verify screen', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('rejects incorrect credentials without creating a session', (
    ...
    await tester.enterText(find.byType(TextFormField).at(0), 'mobiletest@ctms.local');
  ...
  testWidgets('logs in successfully with a verified account and reaches the Camper home', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(const ProviderScope(child: CtmsApp()));
    await tester.pumpAndSettle();
