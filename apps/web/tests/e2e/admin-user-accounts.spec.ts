...
test.describe("Admin user accounts", () => {
	test("searches, views, locks, and unlocks an account", async ({ page }) => {
		...
		await page.route("**/api/users**", async (route) => {
			...
			if (path.endsWith("/lock") && request.method() === "PATCH") {
				status = "suspended";
			}

			if (path.endsWith("/unlock") && request.method() === "PATCH") {
				status = "active";
			}
		...
		await expect(
			page.getByTestId("toast-success").getByText("Đã khóa tài khoản thành công.")
		).toBeVisible();

		await page
			.getByRole("button", {
				name: "Mở khóa Nguyen Camper",
			})
			.click();
		...
		await expect(
			page.getByTestId("toast-success").getByText("Đã mở khóa tài khoản thành công.")
		).toBeVisible();
	});
