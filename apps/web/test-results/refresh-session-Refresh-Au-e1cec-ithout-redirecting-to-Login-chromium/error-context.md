...
- Name: refresh-session.spec.ts >> Refresh Authentication Session — Web Integration (E2E, real backend) >> continues transparently after the access token expires mid-flow, without redirecting to Login
- Location: tests\e2e\refresh-session.spec.ts:81:2
