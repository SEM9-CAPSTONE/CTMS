...
export function checkpointUpdateError(error: unknown): string {
	if (!(error instanceof HttpError))
		return "Không thể cập nhật checkpoint. Vui lòng kiểm tra kết nối và thử lại.";
	const detail = backendDetail(error);
	if (detail) return detail;
	const messages: Record<number, string> = {
		401: "Phiên đăng nhập đã hết hạn.",
		403: "Bạn không có quyền cập nhật checkpoint cho tuyến này.",
		404: "Không tìm thấy tuyến hoặc checkpoint đã chọn.",
		409: "Chỉ có thể cập nhật checkpoint khi tuyến đang ở trạng thái nháp.",
		422: "Dữ liệu checkpoint chưa hợp lệ hoặc vị trí cách tuyến quá 50 mét.",
	};
	return messages[error.status] ?? "Không thể cập nhật checkpoint. Vui lòng thử lại.";
}
