...
**Spec Reference**  
/file/spec/ctms-81-broadcast-emergency-alert-to-trip.md
...
Implement `Broadcast Emergency Alert to Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Broadcast Emergency Alert to Trip`
- Story: As a System, I want to broadcast emergency alert to trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Broadcast Emergency Alert to Trip` within `EPIC 12. SOS and Emergency Management`.
...
The system implements `Broadcast Emergency Alert to Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Broadcast Emergency Alert to Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-262` | Required behavior for `Broadcast Emergency Alert to Trip` must validate and enforce MVP, Host, broadcast, emergency, alert, Trip, type, content, eligible, members as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-081` is the direct scope and acceptance source.
...
### Scenario: Broadcast Emergency Alert to Trip

1. Actor opens or triggers the `Broadcast Emergency Alert to Trip` workflow.
...
The implementation must persist or return only data required for `Broadcast Emergency Alert to Trip`:
...
- The approved PB V3.1 acceptance behavior for `Broadcast Emergency Alert to Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Broadcast Emergency Alert to Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-085-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-085-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Broadcast Emergency Alert to Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-081`
- Logic Subtask: `CTMS-081-T01`
- UI Subtask: `CTMS-081-T02`
...
- Jira Logic Subtask `CTMS-085-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-085-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Broadcast Emergency Alert to Trip`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-85-broadcast-emergency-alert-to-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-85-broadcast-emergency-alert-to-trip.md#ui-and-tests`
