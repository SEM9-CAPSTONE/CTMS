...
**Spec Reference**  
/file/spec/ctms-74-send-sos-alert-in-an-emergency.md
...
Implement `Send SOS Alert in an Emergency` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Send SOS Alert in an Emergency`
- Story: As a System, I want to send sos alert in an emergency so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Send SOS Alert in an Emergency` within `EPIC 12. SOS and Emergency Management`.
...
The system implements `Send SOS Alert in an Emergency` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Send SOS Alert in an Emergency` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-254` | Required behavior for `Send SOS Alert in an Emergency` must validate and enforce SOS, alert, sender, Trip, current, last, location, time, battery, emergency as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-074` is the direct scope and acceptance source.
...
### Scenario: Send SOS Alert in an Emergency

1. Actor opens or triggers the `Send SOS Alert in an Emergency` workflow.
...
The implementation must persist or return only data required for `Send SOS Alert in an Emergency`:
...
- The approved PB V3.1 acceptance behavior for `Send SOS Alert in an Emergency` is implemented as explicit system behavior..
...
- The actor or system attempts `Send SOS Alert in an Emergency`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-077-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-077-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Send SOS Alert in an Emergency` if they are not already implemented?

Affected:
- Jira Story: `CTMS-074`
- Logic Subtask: `CTMS-074-T01`
- UI Subtask: `CTMS-074-T02`
...
- Jira Logic Subtask `CTMS-077-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-077-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Send SOS Alert in an Emergency`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-77-send-sos-alert-in-an-emergency.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-77-send-sos-alert-in-an-emergency.md#ui-and-tests`
