...
**Spec Reference**  
/file/spec/ctms-36-cancel-trip-due-to-insufficient-participants.md
...
Implement `Cancel Trip Due to Insufficient Participants` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Cancel Trip Due to Insufficient Participants`
- Story: As a System, I want to cancel trip due to insufficient participants so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Cancel Trip Due to Insufficient Participants` within `EPIC 4. Trip Management`.
...
- Dependencies are satisfied: CTMS-023, CTMS-032, CTMS-035.
...
The system implements `Cancel Trip Due to Insufficient Participants` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Cancel Trip Due to Insufficient Participants` exactly as approved in the source backlog.
...
| `BR-109` | Required behavior for `Cancel Trip Due to Insufficient Participants` must validate and enforce Trip, cancelled, Booking, cancel, cancellation, policy, refund, notification, commit as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-036` is the direct scope and acceptance source.
...
### Scenario: Cancel Trip Due to Insufficient Participants

1. Actor opens or triggers the `Cancel Trip Due to Insufficient Participants` workflow.
...
If dependency data from `CTMS-023, CTMS-032, CTMS-035` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Cancel Trip Due to Insufficient Participants`:
...
- The approved PB V3.1 acceptance behavior for `Cancel Trip Due to Insufficient Participants` is implemented as explicit system behavior..
...
- The actor or system attempts `Cancel Trip Due to Insufficient Participants`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-036-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-036-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Cancel Trip Due to Insufficient Participants` if they are not already implemented?

Affected:
- Jira Story: `CTMS-036`
- Logic Subtask: `CTMS-036-T01`
- UI Subtask: `CTMS-036-T02`
...
- Jira Logic Subtask `CTMS-036-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-036-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Cancel Trip Due to Insufficient Participants`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-023, CTMS-032, CTMS-035`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-36-cancel-trip-due-to-insufficient-participants.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-36-cancel-trip-due-to-insufficient-participants.md#ui-and-tests`
