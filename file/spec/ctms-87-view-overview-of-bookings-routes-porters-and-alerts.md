...
**Spec Reference**  
/file/spec/ctms-87-view-overview-of-bookings-routes-porters-and-alerts.md
...
Implement `View Overview of Bookings, Routes, Porters, and Alerts` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Overview of Bookings, Routes, Porters, and Alerts`
- Story: As a System, I want to view overview of bookings, routes, porters, and alerts so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Overview of Bookings, Routes, Porters, and Alerts` within `EPIC 14. Host Monitoring Dashboard`.
...
- Dependencies are satisfied: CTMS-031, CTMS-047, CTMS-016, CTMS-074, CTMS-084.
...
The system implements `View Overview of Bookings, Routes, Porters, and Alerts` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Overview of Bookings, Routes, Porters, and Alerts` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-087` is the direct scope and acceptance source.
...
### Scenario: View Overview of Bookings, Routes, Porters, and Alerts

1. Actor opens or triggers the `View Overview of Bookings, Routes, Porters, and Alerts` workflow.
...
If dependency data from `CTMS-031, CTMS-047, CTMS-016, CTMS-077, CTMS-089` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Overview of Bookings, Routes, Porters, and Alerts`:
...
- The approved PB V3.1 acceptance behavior for `View Overview of Bookings, Routes, Porters, and Alerts` is implemented as explicit system behavior..
...
- The actor or system attempts `View Overview of Bookings, Routes, Porters, and Alerts`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-093-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-093-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Overview of Bookings, Routes, Porters, and Alerts` if they are not already implemented?

Affected:
- Jira Story: `CTMS-087`
- Logic Subtask: `CTMS-087-T01`
- UI Subtask: `CTMS-087-T02`
...
- Jira Logic Subtask `CTMS-093-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-093-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Overview of Bookings, Routes, Porters, and Alerts`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-031, CTMS-047, CTMS-016, CTMS-074, CTMS-084`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-93-view-overview-of-bookings-routes-porters-and-alerts.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-93-view-overview-of-bookings-routes-porters-and-alerts.md#ui-and-tests`
