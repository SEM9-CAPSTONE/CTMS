...
Implement `Update Trip Operational Lifecycle and Progress` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Update Trip Operational Lifecycle and Progress`
- Story: As a System, I want to update trip operational lifecycle and progress so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Update Trip Operational Lifecycle and Progress` within `EPIC 4. Trip Management`.
...
- Dependencies are satisfied: CTMS-023, CTMS-011, CTMS-047, CTMS-055.
...
The system implements `Update Trip Operational Lifecycle and Progress` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Update Trip Operational Lifecycle and Progress` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-064` | Required behavior for `Update Trip Operational Lifecycle and Progress` must validate and enforce MVP, Trip, lifecycle, active, flow, draft, pending_approval, published, ongoing, completed as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-065` | Required behavior for `Update Trip Operational Lifecycle and Progress` must validate and enforce trips.starts_at, trips.started_at, Start, Trip, status, published, Host, Lead, Porter, Assignment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-066` | Required behavior for `Update Trip Operational Lifecycle and Progress` must validate and enforce GPS, trips.completed_at, Finish, Trip, status, ongoing, Host, Lead, Porter, Assignment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-234` | Required behavior for `Update Trip Operational Lifecycle and Progress` must validate and enforce GPS, Porter, checkpoint, Trip, assignment, upsert, visits, member, joined, arrival as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-056` is the direct scope and acceptance source.
...
### Scenario: Update Trip Operational Lifecycle and Progress

1. Actor opens or triggers the `Update Trip Operational Lifecycle and Progress` workflow.
...
If dependency data from `CTMS-023, CTMS-011, CTMS-047, CTMS-055` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Update Trip Operational Lifecycle and Progress`:
...
- The approved PB V3.1 acceptance behavior for `Update Trip Operational Lifecycle and Progress` is implemented as explicit system behavior..
...
- The actor or system attempts `Update Trip Operational Lifecycle and Progress`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-056-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-056-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Update Trip Operational Lifecycle and Progress` if they are not already implemented?

Affected:
- Jira Story: `CTMS-056`
- Logic Subtask: `CTMS-056-T01`
- UI Subtask: `CTMS-056-T02`
...
- Jira Logic Subtask `CTMS-056-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-056-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Update Trip Operational Lifecycle and Progress`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-023, CTMS-011, CTMS-047, CTMS-055`
- Status: `To Do`
- Sprint: `Sprint 3`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-56-update-trip-operational-lifecycle-and-progress.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-56-update-trip-operational-lifecycle-and-progress.md#ui-and-tests`
