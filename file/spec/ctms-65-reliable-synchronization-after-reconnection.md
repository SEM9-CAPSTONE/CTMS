...
**Spec Reference**  
/file/spec/ctms-65-reliable-synchronization-after-reconnection.md
...
Implement `Reliable Synchronization after Reconnection` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Reliable Synchronization after Reconnection`
- Story: As a System, I want to reliable synchronization after reconnection so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Reliable Synchronization after Reconnection` within `EPIC 10. Buffer and Synchronization`.
...
- Dependencies are satisfied: CTMS-064.
...
The system implements `Reliable Synchronization after Reconnection` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Reliable Synchronization after Reconnection` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-243` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce GPS, UUID, logs, safety, events, pending, item, synced, failed, Sync as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-244` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce GPS, UUID, safety, event, client, generated, server, upload, idempotent, semantics as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-257` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce SOS, GPS, Synchronization, hazard, events, logs, deterministic, priority, idempotency as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-400` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce Sync, idempotency, upload, batch, idempotency_key, record, stable, local, identifiers, retry as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-401` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce sync_batches, device, session, created_at, client, time, server, received, batch, status as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-404` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce network, client, sync, unsynced, confirmed, safety, events, bulk, historical, gps_logs as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-442` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce record, acknowledgement, sync, batch, stable, identifier, result, accepted, duplicate, failed as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-443` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce synced, record, acknowledgement, accepted, duplicate, retry, failed, retryable, failure_reason, audit as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-190` | OTP TTL, token TTL, attempt limits, rate limits, booking hold time, and retry deadlines must be configuration-driven and must not be hard-coded in business logic. |
...
| `BR-402` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce GPS, validate, ownership, authorization, Trip, member, device, context, sync, payload as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-403` | Required behavior for `Reliable Synchronization after Reconnection` must validate and enforce GPS, validate, schema, coordinate, range, timestamps, package, version, references, synced as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-065` is the direct scope and acceptance source.
...
### Scenario: Reliable Synchronization after Reconnection

1. Actor opens or triggers the `Reliable Synchronization after Reconnection` workflow.
...
If dependency data from `CTMS-065` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Reliable Synchronization after Reconnection`:
...
- The approved PB V3.1 acceptance behavior for `Reliable Synchronization after Reconnection` is implemented as explicit system behavior..
...
- The actor or system attempts `Reliable Synchronization after Reconnection`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-066-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-066-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Reliable Synchronization after Reconnection` if they are not already implemented?

Affected:
- Jira Story: `CTMS-065`
- Logic Subtask: `CTMS-065-T01`
- UI Subtask: `CTMS-065-T02`
...
- Jira Logic Subtask `CTMS-066-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-066-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Reliable Synchronization after Reconnection`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-064`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-66-automatically-sync-gps-logs-after-reconnection.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-66-automatically-sync-gps-logs-after-reconnection.md#ui-and-tests`
