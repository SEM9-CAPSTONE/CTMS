...
Implement `Log In` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Log In`
- Story: As a System, I want to log in so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Log In` within `EPIC 1. Authentication`.
...
- Dependencies are satisfied: CTMS-002.
...
The system implements `Log In` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Log In` exactly as approved in the source backlog.
...
| `BR-170` | Required behavior for `Log In` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-171` | Required behavior for `Log In` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-003` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Log In` workflow.
...
If dependency data from `CTMS-002` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Log In`:
...
- The approved PB V3.1 acceptance behavior for `Log In` is implemented as explicit system behavior..
...
- The actor or system attempts `Log In`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-003-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-003-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Log In` if they are not already implemented?

Affected:
- Jira Story: `CTMS-003`
- Logic Subtask: `CTMS-003-T01`
- UI Subtask: `CTMS-003-T02`
...
- Jira Logic Subtask `CTMS-003-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-003-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Log In`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-03-log-in.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-03-log-in.md#ui-and-tests`
