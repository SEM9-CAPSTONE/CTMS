...
**Spec Reference**  
/file/spec/ctms-45-request-porter-for-trip.md
...
Implement `Request Porter for Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Request Porter for Trip`
- Story: As a System, I want to request porter for trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Request Porter for Trip` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-021, CTMS-043, CTMS-044.
...
The system implements `Request Porter for Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Request Porter for Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-143` | Required behavior for `Request Porter for Trip` must validate and enforce Host, Porter, Request, Trip, request, trip_id, porter_id as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-144` | Required behavior for `Request Porter for Trip` must validate and enforce PENDING, Porter, Request, trip_id, porter_id, request, lifecycle, idempotency, policy as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-145` | Required behavior for `Request Porter for Trip` must validate and enforce D02, Porter, Request, requested_by, authenticated, Host, requested_role, note, day_rate as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-146` | Required behavior for `Request Porter for Trip` must validate and enforce Porter, Request, transaction, commit, notification, preference, policy, failure, rollback, request as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-045` is the direct scope and acceptance source.
...
### Scenario: Request Porter for Trip

1. Actor opens or triggers the `Request Porter for Trip` workflow.
...
If dependency data from `CTMS-021, CTMS-043, CTMS-044` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Request Porter for Trip`:
...
- The approved PB V3.1 acceptance behavior for `Request Porter for Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Request Porter for Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-045-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-045-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Request Porter for Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-045`
- Logic Subtask: `CTMS-045-T01`
- UI Subtask: `CTMS-045-T02`
...
- Jira Logic Subtask `CTMS-045-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-045-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Request Porter for Trip`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-021, CTMS-043, CTMS-044`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Story-level business rules: BR-270, BR-271, BR-273, BR-274
- Jira execution tasks should reference:
  - `/file/spec/ctms-45-request-porter-for-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-45-request-porter-for-trip.md#ui-and-tests`
