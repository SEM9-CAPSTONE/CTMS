...
**Spec Reference**  
/file/spec/ctms-53-download-offline-package-before-departure.md
...
Implement `Download Offline Package before Departure` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Download Offline Package before Departure`
- Story: As a System, I want to download offline package before departure so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Download Offline Package before Departure` within `EPIC 8. Offline Package`.
...
- Dependencies are satisfied: CTMS-052.
...
The system implements `Download Offline Package before Departure` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Download Offline Package before Departure` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-053` is the direct scope and acceptance source.
...
### Scenario: Download Offline Package before Departure

1. Actor opens or triggers the `Download Offline Package before Departure` workflow.
...
If dependency data from `CTMS-052` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Download Offline Package before Departure`:
...
- The approved PB V3.1 acceptance behavior for `Download Offline Package before Departure` is implemented as explicit system behavior..
...
- The actor or system attempts `Download Offline Package before Departure`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-053-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-053-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Download Offline Package before Departure` if they are not already implemented?

Affected:
- Jira Story: `CTMS-053`
- Logic Subtask: `CTMS-053-T01`
- UI Subtask: `CTMS-053-T02`
...
- Jira Logic Subtask `CTMS-053-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-053-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Download Offline Package before Departure`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-052`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-53-download-offline-package-before-departure.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-53-download-offline-package-before-departure.md#ui-and-tests`
