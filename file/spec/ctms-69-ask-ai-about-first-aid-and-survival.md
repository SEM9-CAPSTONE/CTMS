...
**Spec Reference**  
/file/spec/ctms-69-ask-ai-about-first-aid-and-survival.md
...
Implement `Ask AI about First Aid and Survival` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Ask AI about First Aid and Survival`
- Story: As a System, I want to ask ai about first aid and survival so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Ask AI about First Aid and Survival` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-068.
...
The system implements `Ask AI about First Aid and Survival` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Ask AI about First Aid and Survival` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-249` | Required behavior for `Ask AI about First Aid and Survival` must validate and enforce assistant, first, survival, knowledge, base, operational, latency, target, safety, constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-216` | Required behavior for `Ask AI about First Aid and Survival` must validate and enforce RAG, recommendation, override, hard, rule, authoritative, state, route, closed, archived as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-069` is the direct scope and acceptance source.
...
### Scenario: Ask AI about First Aid and Survival

1. Actor opens or triggers the `Ask AI about First Aid and Survival` workflow.
...
If dependency data from `CTMS-071` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Ask AI about First Aid and Survival`:
...
- The approved PB V3.1 acceptance behavior for `Ask AI about First Aid and Survival` is implemented as explicit system behavior..
...
- The actor or system attempts `Ask AI about First Aid and Survival`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-072-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-072-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Ask AI about First Aid and Survival` if they are not already implemented?

Affected:
- Jira Story: `CTMS-069`
- Logic Subtask: `CTMS-069-T01`
- UI Subtask: `CTMS-069-T02`
...
- Jira Logic Subtask `CTMS-072-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-072-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Ask AI about First Aid and Survival`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-068`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-72-ask-ai-about-first-aid-and-survival.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-72-ask-ai-about-first-aid-and-survival.md#ui-and-tests`
