...
**Spec Reference**  
/file/spec/ctms-63-receive-behind-schedule-warning.md
...
Implement `Receive Behind-Schedule Warning` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Receive Behind-Schedule Warning`
- Story: As a System, I want to receive behind-schedule warning so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Receive Behind-Schedule Warning` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-061.
...
The system implements `Receive Behind-Schedule Warning` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Receive Behind-Schedule Warning` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-241` | Required behavior for `Receive Behind-Schedule Warning` must validate and enforce user, checkpoint, policy as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-063` is the direct scope and acceptance source.
...
### Scenario: Receive Behind-Schedule Warning

1. Actor opens or triggers the `Receive Behind-Schedule Warning` workflow.
...
If dependency data from `CTMS-061` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Receive Behind-Schedule Warning`:
...
- The approved PB V3.1 acceptance behavior for `Receive Behind-Schedule Warning` is implemented as explicit system behavior..
...
- The actor or system attempts `Receive Behind-Schedule Warning`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-064-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-064-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Receive Behind-Schedule Warning` if they are not already implemented?

Affected:
- Jira Story: `CTMS-063`
- Logic Subtask: `CTMS-063-T01`
- UI Subtask: `CTMS-063-T02`
...
- Jira Logic Subtask `CTMS-064-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-064-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Receive Behind-Schedule Warning`
- Priority: `Could Have`
- Story points: `8.0`
- Dependencies: `CTMS-061`
- Status: `To Do`
- Sprint: `Sprint 4`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-64-receive-behind-schedule-warning.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-64-receive-behind-schedule-warning.md#ui-and-tests`
