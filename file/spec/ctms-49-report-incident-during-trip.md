...
**Spec Reference**  
/file/spec/ctms-49-report-incident-during-trip.md
...
Implement `Report Incident during Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Report Incident during Trip`
- Story: As a System, I want to report incident during trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Report Incident during Trip` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-048.
...
The system implements `Report Incident during Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Report Incident during Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-161` | Required behavior for `Report Incident during Trip` must validate and enforce Incident, report, Trip, trip_id, reporter, category, description, location, Media, media as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-162` | Required behavior for `Report Incident during Trip` must validate and enforce GPS, incident, safety, event, local, client, generated, idempotency, metadata, sync as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-221` | Required behavior for `Report Incident during Trip` must validate and enforce Incident, safety, event, offline, time, location, client, generated, identifier, local as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-224` | Before committing qualification, Porter Request, or Porter Assignment changes, backend must recheck current Porter/profile state, route qualification, request or assignment state, and schedule conflict rules. |
...
- PB V3.1 row `CTMS-049` is the direct scope and acceptance source.
...
### Scenario: Report Incident during Trip

1. Actor opens or triggers the `Report Incident during Trip` workflow.
...
If dependency data from `CTMS-048` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Report Incident during Trip`:
...
- The approved PB V3.1 acceptance behavior for `Report Incident during Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Report Incident during Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-049-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-049-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Report Incident during Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-049`
- Logic Subtask: `CTMS-049-T01`
- UI Subtask: `CTMS-049-T02`
...
- Jira Logic Subtask `CTMS-049-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-049-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Report Incident during Trip`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-048`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-49-report-incident-during-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-49-report-incident-during-trip.md#ui-and-tests`
