...
**Spec Reference**  
/file/spec/ctms-90-receive-push-notifications-while-app-runs-in-background.md
...
Implement `Receive Push Notifications while App Runs in Background` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Receive Push Notifications while App Runs in Background`
- Story: As a System, I want to receive push notifications while app runs in background so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Receive Push Notifications while App Runs in Background` within `EPIC 15. Notifications`.
...
- Dependencies are satisfied: CTMS-088.
...
The system implements `Receive Push Notifications while App Runs in Background` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Receive Push Notifications while App Runs in Background` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-090` is the direct scope and acceptance source.
...
### Scenario: Receive Push Notifications while App Runs in Background

1. Actor opens or triggers the `Receive Push Notifications while App Runs in Background` workflow.
...
If dependency data from `CTMS-094` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Receive Push Notifications while App Runs in Background`:
...
- The approved PB V3.1 acceptance behavior for `Receive Push Notifications while App Runs in Background` is implemented as explicit system behavior..
...
- The actor or system attempts `Receive Push Notifications while App Runs in Background`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-096-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-096-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Receive Push Notifications while App Runs in Background` if they are not already implemented?

Affected:
- Jira Story: `CTMS-090`
- Logic Subtask: `CTMS-090-T01`
- UI Subtask: `CTMS-090-T02`
...
- Jira Logic Subtask `CTMS-096-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-096-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Receive Push Notifications while App Runs in Background`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-088`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-083, BR-105, BR-106, BR-107, BR-108, BR-194, BR-270, BR-271, BR-273, BR-274
- Jira execution tasks should reference:
  - `/file/spec/ctms-96-receive-push-notifications-while-app-runs-in-background.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-96-receive-push-notifications-while-app-runs-in-background.md#ui-and-tests`
