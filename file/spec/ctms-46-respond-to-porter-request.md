...
Implement `Respond to Porter Request` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Respond to Porter Request`
- Story: As a System, I want to respond to porter request so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Respond to Porter Request` within `EPIC 7. Porter Management`.
...
- Porter: primary actor for this workflow.
...
- Dependencies are satisfied: CTMS-045.
...
The system implements `Respond to Porter Request` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Respond to Porter Request` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-147` | Required behavior for `Respond to Porter Request` must validate and enforce Porter, Accept, Decline, Request, porter_id, status as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-148` | Required behavior for `Respond to Porter Request` must validate and enforce PENDING, ACCEPTED, Accept, Porter, Request, backend, revalidate, availability, qualification, conflict as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-149` | Required behavior for `Respond to Porter Request` must validate and enforce PENDING, DECLINED, Decline, Porter, Request, Assignment, request, decline as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-150` | Required behavior for `Respond to Porter Request` must validate and enforce PENDING, ACCEPTED, DECLINED, CANCELLED, EXPIRED, Porter, Request, expires_at, policy, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-151` | Required behavior for `Respond to Porter Request` must validate and enforce Porter, Request, Host, notification, event, commit, duplicate as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-046` is the direct scope and acceptance source.
...
### Scenario: Respond to Porter Request

1. Actor opens or triggers the `Respond to Porter Request` workflow.
...
If dependency data from `CTMS-045` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Respond to Porter Request`:
...
- The approved PB V3.1 acceptance behavior for `Respond to Porter Request` is implemented as explicit system behavior..
...
- The actor or system attempts `Respond to Porter Request`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-046-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-046-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Respond to Porter Request` if they are not already implemented?

Affected:
- Jira Story: `CTMS-046`
- Logic Subtask: `CTMS-046-T01`
- UI Subtask: `CTMS-046-T02`
...
- Jira Logic Subtask `CTMS-046-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-046-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Respond to Porter Request`
- Priority: `Must Have`
- Story points: `3.0`
- Dependencies: `CTMS-045`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-46-respond-to-porter-request.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-46-respond-to-porter-request.md#ui-and-tests`
