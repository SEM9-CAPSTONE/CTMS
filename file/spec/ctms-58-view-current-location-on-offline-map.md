...
Implement `View Current Location on Offline Map` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Current Location on Offline Map`
- Story: As a System, I want to view current location on offline map so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Current Location on Offline Map` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-055.
...
The system implements `View Current Location on Offline Map` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Current Location on Offline Map` exactly as approved in the source backlog.
...
### Scenario: View Current Location on Offline Map

1. Actor opens or triggers the `View Current Location on Offline Map` workflow.
...
If dependency data from `CTMS-055` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Current Location on Offline Map`:
...
- The approved PB V3.1 acceptance behavior for `View Current Location on Offline Map` is implemented as explicit system behavior..
...
- The actor or system attempts `View Current Location on Offline Map`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-058-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-058-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Current Location on Offline Map` if they are not already implemented?

Affected:
- Jira Story: `CTMS-058`
- Logic Subtask: `CTMS-058-T01`
- UI Subtask: `CTMS-058-T02`
...
- Jira Logic Subtask `CTMS-058-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-058-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Current Location on Offline Map`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-055`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-58-view-current-location-on-offline-map.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-58-view-current-location-on-offline-map.md#ui-and-tests`
