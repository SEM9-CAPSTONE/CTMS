...
**Spec Reference**  
/file/spec/ctms-18-block-new-registrations-when-route-risk-is-red.md
...
Implement `Block New Registrations when Route Risk Is Red` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Block New Registrations when Route Risk Is Red`
- Story: As a System, I want to block new registrations when route risk is red so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Block New Registrations when Route Risk Is Red` within `EPIC 3. Weather Risk Assessment`.
...
- Dependencies are satisfied: CTMS-016.
...
The system implements `Block New Registrations when Route Risk Is Red` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Block New Registrations when Route Risk Is Red` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-046` | Required behavior for `Block New Registrations when Route Risk Is Red` must validate and enforce booking, assessment, route, Trip, level, policy, backend as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-047` | Required behavior for `Block New Registrations when Route Risk Is Red` must validate and enforce booking, Weather, Risk, response, reason, assessment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-018` is the direct scope and acceptance source.
...
### Scenario: Block New Registrations when Route Risk Is Red

1. Actor opens or triggers the `Block New Registrations when Route Risk Is Red` workflow.
...
If dependency data from `CTMS-016` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Block New Registrations when Route Risk Is Red`:
...
- The approved PB V3.1 acceptance behavior for `Block New Registrations when Route Risk Is Red` is implemented as explicit system behavior..
...
- The actor or system attempts `Block New Registrations when Route Risk Is Red`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-018-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-018-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Block New Registrations when Route Risk Is Red` if they are not already implemented?

Affected:
- Jira Story: `CTMS-018`
- Logic Subtask: `CTMS-018-T01`
- UI Subtask: `CTMS-018-T02`
...
- Jira Logic Subtask `CTMS-018-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-018-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Block New Registrations when Route Risk Is Red`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-016`
- Status: `To Do`
- Sprint: `Sprint 2`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-18-block-new-registrations-when-route-risk-is-red.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-18-block-new-registrations-when-route-risk-is-red.md#ui-and-tests`
