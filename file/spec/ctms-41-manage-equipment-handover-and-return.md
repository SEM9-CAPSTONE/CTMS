...
Implement `Manage Equipment Handover and Return` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Equipment Handover and Return`
- Story: As a System, I want to manage equipment handover and return so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Equipment Handover and Return` within `EPIC 6. Equipment and Logistics`.
...
- Dependencies are satisfied: CTMS-040, CTMS-037.
...
The system implements `Manage Equipment Handover and Return` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Equipment Handover and Return` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-130` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce giao, reservation, reserved, picked_up, picked_up_at, handed_over_by, idempotent as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-132` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce booking, Trip, user, account, handed_over_by, Host, Porter, giao, server as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-133` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce good_returned_quantity, damaged_quantity, lost_quantity, outstanding_quantity, returned_at, condition, summary, quantity, outcome, good_returned as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-134` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce MVP, damaged_quantity, condition, note, rental, available, quantity, Host, inventory, adjustment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-135` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce lost_quantity, available, quantity, quantity_total, inventory, adjustment, Host, Admin, audit, client as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-136` | Required behavior for `Manage Equipment Handover and Return` must validate and enforce Rental, outstanding_quantity, not_returned, manual, flow, idempotent, outstanding, quantity, available, adjustment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-041` is the direct scope and acceptance source.
...
### Scenario: Manage Equipment Handover and Return

1. Actor opens or triggers the `Manage Equipment Handover and Return` workflow.
...
If dependency data from `CTMS-040, CTMS-037` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Manage Equipment Handover and Return`:
...
- The approved PB V3.1 acceptance behavior for `Manage Equipment Handover and Return` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Equipment Handover and Return`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-041-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-041-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Equipment Handover and Return` if they are not already implemented?

Affected:
- Jira Story: `CTMS-041`
- Logic Subtask: `CTMS-041-T01`
- UI Subtask: `CTMS-041-T02`
...
- Jira Logic Subtask `CTMS-041-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-041-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Equipment Handover and Return`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-040, CTMS-037`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-41-manage-equipment-handover-and-return.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-41-manage-equipment-handover-and-return.md#ui-and-tests`
