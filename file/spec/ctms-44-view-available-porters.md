...
**Spec Reference**  
/file/spec/ctms-44-view-available-porters.md
...
Implement `View Available Porters` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Available Porters`
- Story: As a System, I want to view available porters so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Available Porters` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-043.
...
The system implements `View Available Porters` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Available Porters` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-141` | Required behavior for `View Available Porters` must validate and enforce Porter, Trip, account, profile, active, availability, experience, assignment, route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-142` | Required behavior for `View Available Porters` must validate and enforce Porter, assignment, overlap, work_range, Trip, backend, request as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-220` | Required behavior for `View Available Porters` must validate and enforce Porter, eligibility, availability, qualification, assignment, conflicts as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-044` is the direct scope and acceptance source.
...
### Scenario: View Available Porters

1. Actor opens or triggers the `View Available Porters` workflow.
...
If dependency data from `CTMS-043` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Available Porters`:
...
- The approved PB V3.1 acceptance behavior for `View Available Porters` is implemented as explicit system behavior..
...
- The actor or system attempts `View Available Porters`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-044-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-044-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Available Porters` if they are not already implemented?

Affected:
- Jira Story: `CTMS-044`
- Logic Subtask: `CTMS-044-T01`
- UI Subtask: `CTMS-044-T02`
...
- Jira Logic Subtask `CTMS-044-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-044-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Available Porters`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-043`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-44-view-available-porters.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-44-view-available-porters.md#ui-and-tests`
