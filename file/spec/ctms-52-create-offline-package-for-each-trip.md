...
**Spec Reference**  
/file/spec/ctms-52-create-offline-package-for-each-trip.md
...
Implement `Create Offline Package for Each Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Offline Package for Each Trip`
- Story: As a System, I want to create offline package for each trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Offline Package for Each Trip` within `EPIC 8. Offline Package`.
...
- Dependencies are satisfied: CTMS-011, CTMS-014, CTMS-016, CTMS-023, CTMS-030, CTMS-032, CTMS-042, CTMS-047, CTMS-051.
...
The system implements `Create Offline Package for Each Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Offline Package for Each Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-229` | Required behavior for `Create Offline Package for Each Trip` must validate and enforce Offline, package, Trip, published, Route, geometry, version, checkpoints, route_hazard_areas, trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-365` | When authoritative Route, checkpoint, hazard, or safety instruction data changes in a way that affects Offline Safety Packages, the system must create a new package version. Existing packages must not be modified in place. |
| `BR-428` | Required behavior for `Create Offline Package for Each Trip` must validate and enforce Package, generation, snapshot, reference, versions, authoritative, inputs, version, chung, Route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-052` is the direct scope and acceptance source.
...
### Scenario: Create Offline Package for Each Trip

1. Actor opens or triggers the `Create Offline Package for Each Trip` workflow.
...
If dependency data from `CTMS-011, CTMS-014, CTMS-016, CTMS-023, CTMS-030, CTMS-032, CTMS-042, CTMS-047, CTMS-051` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Create Offline Package for Each Trip`:
...
- The approved PB V3.1 acceptance behavior for `Create Offline Package for Each Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Offline Package for Each Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-052-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-052-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create Offline Package for Each Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-052`
- Logic Subtask: `CTMS-052-T01`
- UI Subtask: `CTMS-052-T02`
...
- Jira Logic Subtask `CTMS-052-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-052-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Offline Package for Each Trip`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-011, CTMS-014, CTMS-016, CTMS-023, CTMS-030, CTMS-032, CTMS-042, CTMS-047, CTMS-051`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-52-create-offline-package-for-each-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-52-create-offline-package-for-each-trip.md#ui-and-tests`
