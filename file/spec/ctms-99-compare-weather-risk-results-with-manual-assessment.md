...
**Spec Reference**  
/file/spec/ctms-99-compare-weather-risk-results-with-manual-assessment.md
...
Implement `Compare Weather Risk Results with Manual Assessment` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Compare Weather Risk Results with Manual Assessment`
- Story: As a System, I want to compare weather risk results with manual assessment so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Compare Weather Risk Results with Manual Assessment` within `EPIC 17. Reports and Evaluation Metrics`.
...
The system implements `Compare Weather Risk Results with Manual Assessment` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Compare Weather Risk Results with Manual Assessment` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-280` | Required behavior for `Compare Weather Risk Results with Manual Assessment` must validate and enforce Weather, Risk, evaluation, manual, assessment, test, version, Release, gates, overall as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-099` is the direct scope and acceptance source.
...
### Scenario: Compare Weather Risk Results with Manual Assessment

1. Actor opens or triggers the `Compare Weather Risk Results with Manual Assessment` workflow.
...
The implementation must persist or return only data required for `Compare Weather Risk Results with Manual Assessment`:
...
- The approved PB V3.1 acceptance behavior for `Compare Weather Risk Results with Manual Assessment` is implemented as explicit system behavior..
...
- The actor or system attempts `Compare Weather Risk Results with Manual Assessment`.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-105-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Compare Weather Risk Results with Manual Assessment` if they are not already implemented?

Affected:
- Jira Story: `CTMS-099`
- Logic Subtask: `CTMS-099-T01`
- UI Subtask: `CTMS-099-T02`
...
- Jira Logic Subtask `CTMS-105-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-105-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Compare Weather Risk Results with Manual Assessment`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-105-compare-weather-risk-results-with-manual-assessment.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-105-compare-weather-risk-results-with-manual-assessment.md#ui-and-tests`
