...
**Spec Reference**  
/file/spec/ctms-50-manage-survival-documents.md
...
Implement `Manage Survival Documents` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Survival Documents`
- Story: As a System, I want to manage survival documents so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Survival Documents` within `EPIC 11. AI Survival Assistant and RAG`.
...
The system implements `Manage Survival Documents` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Survival Documents` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-227` | Required behavior for `Manage Survival Documents` must validate and enforce Survival, document, upload, classification, review, publication, versioning, lifecycle, status, knowledge as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-050` is the direct scope and acceptance source.
...
### Scenario: Manage Survival Documents

1. Actor opens or triggers the `Manage Survival Documents` workflow.
...
The implementation must persist or return only data required for `Manage Survival Documents`:
...
- The approved PB V3.1 acceptance behavior for `Manage Survival Documents` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Survival Documents`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-050-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-050-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Survival Documents` if they are not already implemented?

Affected:
- Jira Story: `CTMS-050`
- Logic Subtask: `CTMS-050-T01`
- UI Subtask: `CTMS-050-T02`
...
- Jira Logic Subtask `CTMS-050-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-050-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Survival Documents`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-50-manage-survival-documents.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-50-manage-survival-documents.md#ui-and-tests`
