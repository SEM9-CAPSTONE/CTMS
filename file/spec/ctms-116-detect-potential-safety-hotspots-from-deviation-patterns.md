...
**Spec Reference**  
/file/spec/ctms-116-detect-potential-safety-hotspots-from-deviation-patterns.md
...
Implement `Detect Potential Safety Hotspots from Deviation Patterns` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Detect Potential Safety Hotspots from Deviation Patterns`
- Story: As a System, I want to detect potential safety hotspots from deviation patterns so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Detect Potential Safety Hotspots from Deviation Patterns` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-115.
...
The system implements `Detect Potential Safety Hotspots from Deviation Patterns` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Detect Potential Safety Hotspots from Deviation Patterns` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-355` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce PotentialSafetyHotspot, derived, safety, candidate, historical, RouteDeviationEvent, authoritative, hazard as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-356` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce GPS, analytics, Route, geometry, checkpoint, route_hazard_areas, safety, instruction, pattern, suggestion as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-417` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce Hotspot, detection, rolling, window, Candidate, Route, cluster, distinct, Trips, Members as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-418` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce RouteDeviationEvent, Trip, Member, distinct, nhau, hotspot as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-420` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce Hotspot, candidate, analysis_version, config_version, reviewer, logic as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-421` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce candidate, spatial, overlap, merge, Route, version, evidence, window, policy, dedup as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-422` | Required behavior for `Detect Potential Safety Hotspots from Deviation Patterns` must validate and enforce DISMISSED, hotspot, authoritative, hazard, reopen, candidate, explicit, detection, policy, dismissal as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-116` is the direct scope and acceptance source.
...
### Scenario: Detect Potential Safety Hotspots from Deviation Patterns

1. Actor opens or triggers the `Detect Potential Safety Hotspots from Deviation Patterns` workflow.
...
If dependency data from `CTMS-059, CTMS-060, CTMS-066` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Detect Potential Safety Hotspots from Deviation Patterns`:
...
- The approved PB V3.1 acceptance behavior for `Detect Potential Safety Hotspots from Deviation Patterns` is implemented as explicit system behavior..
...
- The actor or system attempts `Detect Potential Safety Hotspots from Deviation Patterns`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-122-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-122-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Detect Potential Safety Hotspots from Deviation Patterns` if they are not already implemented?

Affected:
- Jira Story: `CTMS-116`
- Logic Subtask: `CTMS-116-T01`
- UI Subtask: `CTMS-116-T02`
...
- Jira Logic Subtask `CTMS-122-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-122-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Detect Potential Safety Hotspots from Deviation Patterns`
- Priority: `Should Have`
- Story points: `13.0`
- Dependencies: `CTMS-115`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
- Planned window: `46285` to `46298`
...
- Story-level business rules: BR-231, BR-233, BR-235, BR-237, BR-365, BR-371, BR-372, BR-437, BR-438, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-122-detect-potential-safety-hotspots-from-deviation-patterns.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-122-detect-potential-safety-hotspots-from-deviation-patterns.md#ui-and-tests`
