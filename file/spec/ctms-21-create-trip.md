...
**Spec Reference**  
/file/spec/ctms-21-create-trip.md
...
Implement `Create Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Trip`
- Story: As a System, I want to create trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Trip` within `EPIC 4. Trip Management`.
...
The system implements `Create Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-054` | Required behavior for `Create Trip` must validate and enforce Trip, approved, Route, version, host_id, Host, title, trip_type, duration_nights as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-055` | Required behavior for `Create Trip` must validate and enforce NULL, Trip, starts_at, ends_at, booking_deadline, meeting_at, capacity_min, capacity_max, seats_taken, is_free as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-056` | Required behavior for `Create Trip` must validate and enforce Trip, status, draft, Host, trip_waypoints, submit, pending_approval as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-021` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Create Trip` workflow.
...
The implementation must persist or return only data required for `Create Trip`:
...
- The approved PB V3.1 acceptance behavior for `Create Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-021-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-021-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-021`
- Logic Subtask: `CTMS-021-T01`
- UI Subtask: `CTMS-021-T02`
...
- Jira Logic Subtask `CTMS-021-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-021-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Trip`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-21-create-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-21-create-trip.md#ui-and-tests`
