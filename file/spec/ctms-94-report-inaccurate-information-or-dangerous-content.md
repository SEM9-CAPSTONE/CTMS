...
**Spec Reference**  
/file/spec/ctms-94-report-inaccurate-information-or-dangerous-content.md
...
Implement `Report Inaccurate Information or Dangerous Content` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Report Inaccurate Information or Dangerous Content`
- Story: As a System, I want to report inaccurate information or dangerous content so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Report Inaccurate Information or Dangerous Content` within `EPIC 16. Reviews and Feedback`.
...
The system implements `Report Inaccurate Information or Dangerous Content` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Report Inaccurate Information or Dangerous Content` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-275` | Required behavior for `Report Inaccurate Information or Dangerous Content` must validate and enforce Report, supported, report, type, target, description, trackable, status, Admin, moderation as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-094` is the direct scope and acceptance source.
...
### Scenario: Report Inaccurate Information or Dangerous Content

1. Actor opens or triggers the `Report Inaccurate Information or Dangerous Content` workflow.
...
The implementation must persist or return only data required for `Report Inaccurate Information or Dangerous Content`:
...
- The approved PB V3.1 acceptance behavior for `Report Inaccurate Information or Dangerous Content` is implemented as explicit system behavior..
...
- The actor or system attempts `Report Inaccurate Information or Dangerous Content`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-100-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-100-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Report Inaccurate Information or Dangerous Content` if they are not already implemented?

Affected:
- Jira Story: `CTMS-094`
- Logic Subtask: `CTMS-094-T01`
- UI Subtask: `CTMS-094-T02`
...
- Jira Logic Subtask `CTMS-100-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-100-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Report Inaccurate Information or Dangerous Content`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-100-report-inaccurate-information-or-dangerous-content.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-100-report-inaccurate-information-or-dangerous-content.md#ui-and-tests`
