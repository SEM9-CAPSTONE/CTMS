...
**Spec Reference**  
/file/spec/ctms-62-view-direction-and-distance-back-to-route.md
...
Implement `View Direction and Distance Back to Route` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Direction and Distance Back to Route`
- Story: As a System, I want to view direction and distance back to route so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Direction and Distance Back to Route` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-060.
...
The system implements `View Direction and Distance Back to Route` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Direction and Distance Back to Route` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-240` | Off-route detection must calculate `distance_to_route` on-device from the GPS sample and the approved Route in the active package. One sample over threshold is only suspected off-route; by default, at least 3 consecutive valid over-threshold samples are required to confirm `OFF_ROUTE`, unless an approved stricter safety policy applies. |
...
- PB V3.1 row `CTMS-062` is the direct scope and acceptance source.
...
### Scenario: View Direction and Distance Back to Route

1. Actor opens or triggers the `View Direction and Distance Back to Route` workflow.
...
The implementation must persist or return only data required for `View Direction and Distance Back to Route`:
...
- The approved PB V3.1 acceptance behavior for `View Direction and Distance Back to Route` is implemented as explicit system behavior..
...
- The actor or system attempts `View Direction and Distance Back to Route`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-062-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-062-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Direction and Distance Back to Route` if they are not already implemented?

Affected:
- Jira Story: `CTMS-062`
- Logic Subtask: `CTMS-062-T01`
- UI Subtask: `CTMS-062-T02`
...
- Jira Logic Subtask `CTMS-062-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-062-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Direction and Distance Back to Route`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-060`
- Status: `To Do`
- Sprint: `Sprint 4`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-62-view-direction-and-distance-back-to-route.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-62-view-direction-and-distance-back-to-route.md#ui-and-tests`
