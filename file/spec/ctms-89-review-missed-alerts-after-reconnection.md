...
**Spec Reference**  
/file/spec/ctms-89-review-missed-alerts-after-reconnection.md
...
Implement `Review Missed Alerts after Reconnection` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Review Missed Alerts after Reconnection`
- Story: As a System, I want to review missed alerts after reconnection so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Review Missed Alerts after Reconnection` within `EPIC 12. SOS and Emergency Management`.
...
- Dependencies are satisfied: CTMS-081, CTMS-088.
...
The system implements `Review Missed Alerts after Reconnection` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Review Missed Alerts after Reconnection` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-272` | Required behavior for `Review Missed Alerts after Reconnection` must validate and enforce reconnect, client, eligible, unread, missed, alerts, duplicate, original, alert, event as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-089` is the direct scope and acceptance source.
...
### Scenario: Review Missed Alerts after Reconnection

1. Actor opens or triggers the `Review Missed Alerts after Reconnection` workflow.
...
If dependency data from `CTMS-085, CTMS-094` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Review Missed Alerts after Reconnection`:
...
- The approved PB V3.1 acceptance behavior for `Review Missed Alerts after Reconnection` is implemented as explicit system behavior..
...
- The actor or system attempts `Review Missed Alerts after Reconnection`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-095-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-095-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Review Missed Alerts after Reconnection` if they are not already implemented?

Affected:
- Jira Story: `CTMS-089`
- Logic Subtask: `CTMS-089-T01`
- UI Subtask: `CTMS-089-T02`
...
- Jira Logic Subtask `CTMS-095-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-095-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Review Missed Alerts after Reconnection`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-081, CTMS-088`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-95-review-missed-alerts-after-reconnection.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-95-review-missed-alerts-after-reconnection.md#ui-and-tests`
