...
Implement `Reset Forgotten Password` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Reset Forgotten Password`
- Story: As a System, I want to reset forgotten password so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Reset Forgotten Password` within `EPIC 1. Authentication`.
...
- Dependencies are satisfied: CTMS-001.
...
The system implements `Reset Forgotten Password` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Reset Forgotten Password` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-014` | Password reset is allowed only after ownership is verified with a valid, correct-purpose, unused OTP or reset token/link. |
| `BR-015` | The new password must satisfy the active password policy and must never be stored or logged in plaintext. |
| `BR-016` | After a successful password reset, all active refresh tokens for the user must be revoked in the same business operation or immediately after commit through a consistent mechanism. |
| `BR-171` | Required behavior for `Reset Forgotten Password` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-005` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Reset Forgotten Password` workflow.
...
If dependency data from `CTMS-001` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Reset Forgotten Password`:
...
- The approved PB V3.1 acceptance behavior for `Reset Forgotten Password` is implemented as explicit system behavior..
...
- The actor or system attempts `Reset Forgotten Password`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-005-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-005-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Reset Forgotten Password` if they are not already implemented?

Affected:
- Jira Story: `CTMS-005`
- Logic Subtask: `CTMS-005-T01`
- UI Subtask: `CTMS-005-T02`
...
- Jira Logic Subtask `CTMS-005-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-005-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Reset Forgotten Password`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-05-reset-forgotten-password.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-05-reset-forgotten-password.md#ui-and-tests`
