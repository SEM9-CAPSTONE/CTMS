...
Implement `Add Members to Booking` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Add Members to Booking`
- Story: As a System, I want to add members to booking so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Add Members to Booking` within `EPIC 5. Booking and Payment`.
...
The system implements `Add Members to Booking` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Add Members to Booking` exactly as approved in the source backlog.
...
### Scenario: Add Members to Booking

1. Actor opens or triggers the `Add Members to Booking` workflow.
...
The implementation must persist or return only data required for `Add Members to Booking`:
...
- The approved PB V3.1 acceptance behavior for `Add Members to Booking` is implemented as explicit system behavior..
...
- The actor or system attempts `Add Members to Booking`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-030-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-030-T02`; backend/server responses remain the source of truth for server-owned business state.
...
- Jira Logic Subtask `CTMS-030-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-030-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Add Members to Booking`
- Priority: `Must Have`
- Story points: `5.0`
