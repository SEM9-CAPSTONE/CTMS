...
**Spec Reference**  
/file/spec/ctms-12-close-or-reopen-route-based-on-conditions.md
...
Implement `Close or Reopen Route Based on Conditions` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Close or Reopen Route Based on Conditions`
- Story: As a System, I want to close or reopen route based on conditions so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Close or Reopen Route Based on Conditions` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
- Dependencies are satisfied: CTMS-013.
...
The system implements `Close or Reopen Route Based on Conditions` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Close or Reopen Route Based on Conditions` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-029` | Required behavior for `Close or Reopen Route Based on Conditions` must validate and enforce Route, status, closed, archived, submit, publish, Trip, booking, backend, route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-030` | Required behavior for `Close or Reopen Route Based on Conditions` must validate and enforce route, Trip, published, ongoing, audit, Host, Porter, Camper, Booking as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-032` | Required behavior for `Close or Reopen Route Based on Conditions` must validate and enforce Route, status, closed, actor, audit, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-012` is the direct scope and acceptance source.
...
### Scenario: Close or Reopen Route Based on Conditions

1. Actor opens or triggers the `Close or Reopen Route Based on Conditions` workflow.
...
If dependency data from `CTMS-013` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Close or Reopen Route Based on Conditions`:
...
- The approved PB V3.1 acceptance behavior for `Close or Reopen Route Based on Conditions` is implemented as explicit system behavior..
...
- The actor or system attempts `Close or Reopen Route Based on Conditions`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-012-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-012-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Close or Reopen Route Based on Conditions` if they are not already implemented?

Affected:
- Jira Story: `CTMS-012`
- Logic Subtask: `CTMS-012-T01`
- UI Subtask: `CTMS-012-T02`
...
- Jira Logic Subtask `CTMS-012-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-012-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Close or Reopen Route Based on Conditions`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-013`
- Status: `To Do`
- Sprint: `Sprint 2`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-12-close-or-reopen-route-based-on-conditions.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-12-close-or-reopen-route-based-on-conditions.md#ui-and-tests`
