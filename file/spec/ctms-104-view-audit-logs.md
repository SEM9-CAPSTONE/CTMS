...
**Spec Reference**  
/file/spec/ctms-104-view-audit-logs.md
...
Implement `View Audit Logs` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Audit Logs`
- Story: As a System, I want to view audit logs so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Audit Logs` within `EPIC 18. Administration and Audit`.
...
- Dependencies are satisfied: CTMS-103.
...
The system implements `View Audit Logs` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Audit Logs` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-166` | Required behavior for `View Audit Logs` must validate and enforce API, audit_logs, append, only, User, Admin, audit, record as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-286` | Required behavior for `View Audit Logs` must validate and enforce Admin, audit, logs, actor, user, action, target, record as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-104` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `View Audit Logs` workflow.
...
If dependency data from `CTMS-109` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Audit Logs`:
...
- The approved PB V3.1 acceptance behavior for `View Audit Logs` is implemented as explicit system behavior..
...
- The actor or system attempts `View Audit Logs`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-110-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-110-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Audit Logs` if they are not already implemented?

Affected:
- Jira Story: `CTMS-104`
- Logic Subtask: `CTMS-104-T01`
- UI Subtask: `CTMS-104-T02`
...
- Jira Logic Subtask `CTMS-110-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-110-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Audit Logs`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-103`
- Status: `To Do`
- Sprint: `Sprint 1`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-110-view-audit-logs.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-110-view-audit-logs.md#ui-and-tests`
