...
**Spec Reference**  
/file/spec/ctms-101-view-booking-and-trip-performance-reports.md
...
Implement `View Booking and Trip Performance Reports` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Booking and Trip Performance Reports`
- Story: As a System, I want to view booking and trip performance reports so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Booking and Trip Performance Reports` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-029, CTMS-034, CTMS-038.
...
The system implements `View Booking and Trip Performance Reports` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Booking and Trip Performance Reports` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-283` | Required behavior for `View Booking and Trip Performance Reports` must validate and enforce Authorized, performance, reports, filter, time, Trip, province, city, snapshot, route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-101` is the direct scope and acceptance source.
...
### Scenario: View Booking and Trip Performance Reports

1. Actor opens or triggers the `View Booking and Trip Performance Reports` workflow.
...
If dependency data from `CTMS-029, CTMS-034, CTMS-038` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Booking and Trip Performance Reports`:
...
- The approved PB V3.1 acceptance behavior for `View Booking and Trip Performance Reports` is implemented as explicit system behavior..
...
- The actor or system attempts `View Booking and Trip Performance Reports`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-107-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-107-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Booking and Trip Performance Reports` if they are not already implemented?

Affected:
- Jira Story: `CTMS-101`
- Logic Subtask: `CTMS-101-T01`
- UI Subtask: `CTMS-101-T02`
...
- Jira Logic Subtask `CTMS-107-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-107-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Booking and Trip Performance Reports`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-029, CTMS-034, CTMS-038`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-107-view-booking-and-trip-performance-reports.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-107-view-booking-and-trip-performance-reports.md#ui-and-tests`
