...
**Spec Reference**  
/file/spec/ctms-10-create-trekking-route-on-map.md
...
Implement `Create Trekking Route on Map` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Trekking Route on Map`
- Story: As a System, I want to create trekking route on map so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Trekking Route on Map` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
The system implements `Create Trekking Route on Map` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Trekking Route on Map` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-027` | Required behavior for `Create Trekking Route on Map` must validate and enforce route.host_id, Trekking, Route, Host, route, host_id, name, route_geom, LineString, start_location as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-010` is the direct scope and acceptance source.
...
### Scenario: Create Trekking Route on Map

1. Actor opens or triggers the `Create Trekking Route on Map` workflow.
...
The implementation must persist or return only data required for `Create Trekking Route on Map`:
...
- The approved PB V3.1 acceptance behavior for `Create Trekking Route on Map` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Trekking Route on Map`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-010-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-010-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create Trekking Route on Map` if they are not already implemented?

Affected:
- Jira Story: `CTMS-010`
- Logic Subtask: `CTMS-010-T01`
- UI Subtask: `CTMS-010-T02`
...
- Jira Logic Subtask `CTMS-010-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-010-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Trekking Route on Map`
- Priority: `Must Have`
- Story points: `13.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-10-create-trekking-route-on-map.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-10-create-trekking-route-on-map.md#ui-and-tests`
