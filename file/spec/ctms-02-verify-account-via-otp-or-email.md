...
Implement `Verify Account via OTP or Email` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Verify Account via OTP or Email`
- Story: As a System, I want to verify account via otp or email so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Verify Account via OTP or Email` within `EPIC 1. Authentication`.
...
The system implements `Verify Account via OTP or Email` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Verify Account via OTP or Email` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-002` is the direct scope and acceptance source.
...
### Scenario: Verify Account via OTP or Email

1. Actor opens or triggers the `Verify Account via OTP or Email` workflow.
...
The implementation must persist or return only data required for `Verify Account via OTP or Email`:
...
- The approved PB V3.1 acceptance behavior for `Verify Account via OTP or Email` is implemented as explicit system behavior..
...
- The actor or system attempts `Verify Account via OTP or Email`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-002-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-002-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Verify Account via OTP or Email` if they are not already implemented?

Affected:
- Jira Story: `CTMS-002`
- Logic Subtask: `CTMS-002-T01`
- UI Subtask: `CTMS-002-T02`
...
- Jira Logic Subtask `CTMS-002-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-002-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Verify Account via OTP or Email`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-02-verify-account-via-otp-or-email.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-02-verify-account-via-otp-or-email.md#ui-and-tests`
