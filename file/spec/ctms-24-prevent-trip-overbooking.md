...
**Spec Reference**  
/file/spec/ctms-24-prevent-trip-overbooking.md
...
Implement `Prevent Trip Overbooking` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Prevent Trip Overbooking`
- Story: As a System, I want to prevent trip overbooking so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Prevent Trip Overbooking` within `EPIC 4. Trip Management`.
...
- Dependencies are satisfied: CTMS-021.
...
The system implements `Prevent Trip Overbooking` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Prevent Trip Overbooking` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-067` | Required behavior for `Prevent Trip Overbooking` must validate and enforce cancel, expire, booking, seats_taken, transaction, rollback, seat, counters as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-068` | Required behavior for `Prevent Trip Overbooking` must validate and enforce seats_taken, backend, serialize, concurrent, changes, trip_id, advisory, lock as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-069` | Required behavior for `Prevent Trip Overbooking` must validate and enforce seats_taken, booking, policy, pending_payment, confirmed, cancelled, expired, completed, overbooking, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-070` | Required behavior for `Prevent Trip Overbooking` must validate and enforce confirmed_participant_count, num_people, Booking, status, confirmed, pending_payment, capacity_min, seats_taken as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-071` | Required behavior for `Prevent Trip Overbooking` must validate and enforce trips.capacity_max, Booking, num_people, seats_taken, trips, capacity_max, capacity_min, Trip, booking as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-072` | Required behavior for `Prevent Trip Overbooking` must validate and enforce transaction, booking, conflict, deadlock, serialization, failure, rollback, retry, policy, seats_taken as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-024` is the direct scope and acceptance source.
...
### Scenario: Prevent Trip Overbooking

1. Actor opens or triggers the `Prevent Trip Overbooking` workflow.
...
If dependency data from `CTMS-021` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Prevent Trip Overbooking`:
...
- The approved PB V3.1 acceptance behavior for `Prevent Trip Overbooking` is implemented as explicit system behavior..
...
- The actor or system attempts `Prevent Trip Overbooking`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-024-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-024-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Prevent Trip Overbooking` if they are not already implemented?

Affected:
- Jira Story: `CTMS-024`
- Logic Subtask: `CTMS-024-T01`
- UI Subtask: `CTMS-024-T02`
...
- Jira Logic Subtask `CTMS-024-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-024-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Prevent Trip Overbooking`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-021`
- Status: `To Do`
- Sprint: `Sprint 3`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-24-prevent-trip-overbooking.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-24-prevent-trip-overbooking.md#ui-and-tests`
