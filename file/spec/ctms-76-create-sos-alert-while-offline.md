...
**Spec Reference**  
/file/spec/ctms-76-create-sos-alert-while-offline.md
...
Implement `Create SOS Alert while Offline` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create SOS Alert while Offline`
- Story: As a System, I want to create sos alert while offline so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create SOS Alert while Offline` within `EPIC 12. SOS and Emergency Management`.
...
- Dependencies are satisfied: CTMS-064, CTMS-065, CTMS-074.
...
The system implements `Create SOS Alert while Offline` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create SOS Alert while Offline` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-256` | Required behavior for `Create SOS Alert while Offline` must validate and enforce SOS, offline, local, sync, priority, safety, guidance, duplicate as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-257` | Required behavior for `Create SOS Alert while Offline` must validate and enforce SOS, GPS, Synchronization, hazard, events, logs, deterministic, priority, idempotency as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-076` is the direct scope and acceptance source.
...
### Scenario: Create SOS Alert while Offline

1. Actor opens or triggers the `Create SOS Alert while Offline` workflow.
...
If dependency data from `CTMS-077, CTMS-065` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Create SOS Alert while Offline`:
...
- The approved PB V3.1 acceptance behavior for `Create SOS Alert while Offline` is implemented as explicit system behavior..
...
- The actor or system attempts `Create SOS Alert while Offline`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-079-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-079-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create SOS Alert while Offline` if they are not already implemented?

Affected:
- Jira Story: `CTMS-076`
- Logic Subtask: `CTMS-076-T01`
- UI Subtask: `CTMS-076-T02`
...
- Jira Logic Subtask `CTMS-079-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-079-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create SOS Alert while Offline`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-064, CTMS-065, CTMS-074`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-79-create-sos-alert-while-offline.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-79-create-sos-alert-while-offline.md#ui-and-tests`
