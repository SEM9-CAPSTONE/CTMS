...
**Spec Reference**  
/file/spec/ctms-78-establish-authenticated-websocket-connection.md
...
Implement `Establish Authenticated WebSocket Connection` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Establish Authenticated WebSocket Connection`
- Story: As a System, I want to establish authenticated websocket connection so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Establish Authenticated WebSocket Connection` within `EPIC 13. Real-Time Communication`.
...
The system implements `Establish Authenticated WebSocket Connection` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Establish Authenticated WebSocket Connection` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-078` is the direct scope and acceptance source.
...
### Scenario: Establish Authenticated WebSocket Connection

1. Actor opens or triggers the `Establish Authenticated WebSocket Connection` workflow.
...
The implementation must persist or return only data required for `Establish Authenticated WebSocket Connection`:
...
- The approved PB V3.1 acceptance behavior for `Establish Authenticated WebSocket Connection` is implemented as explicit system behavior..
...
- The actor or system attempts `Establish Authenticated WebSocket Connection`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-082-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-082-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Establish Authenticated WebSocket Connection` if they are not already implemented?

Affected:
- Jira Story: `CTMS-078`
- Logic Subtask: `CTMS-078-T01`
- UI Subtask: `CTMS-078-T02`
...
- Jira Logic Subtask `CTMS-082-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-082-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Establish Authenticated WebSocket Connection`
- Priority: `Must Have`
- Story points: `8.0`
...
- Story-level business rules: BR-049, BR-218, BR-252, BR-253, BR-255, BR-270, BR-271, BR-273, BR-274
- Jira execution tasks should reference:
  - `/file/spec/ctms-82-establish-authenticated-websocket-connection.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-82-establish-authenticated-websocket-connection.md#ui-and-tests`
