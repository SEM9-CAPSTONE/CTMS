...
**Spec Reference**  
/file/spec/ctms-43-manage-porter-profile-and-route-qualification.md
...
Implement `Manage Porter Profile and Route Qualification` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Porter Profile and Route Qualification`
- Story: As a System, I want to manage porter profile and route qualification so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Porter Profile and Route Qualification` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-006, CTMS-013.
...
The system implements `Manage Porter Profile and Route Qualification` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Porter Profile and Route Qualification` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-139` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce porter_profiles, experience_years, certifications, languages, availability_status, rating_avg, completed_trips, schema, Porter, Profile as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-140` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce Porter, actor, Host, Request, Assignment, Trip, membership, resource, trung as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-152` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce Route, qualification, Porter, porter_id, route_id, active, profile, porter_routes, minh as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-153` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce porter_routes.proficiency, porter_routes, proficiency, learning, proficient, expert, lead as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-154` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce minh, proficiency, times_led, verified_by, verified_at, authenticated, Host, Route, Admin, Porter as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-155` | Required behavior for `Manage Porter Profile and Route Qualification` must validate and enforce Unique, constraint, porter_id, route_id, duplicate, current, proficiency, record, history as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-043` is the direct scope and acceptance source.
...
### Scenario: Manage Porter Profile and Route Qualification

1. Actor opens or triggers the `Manage Porter Profile and Route Qualification` workflow.
...
If dependency data from `CTMS-006, CTMS-013` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Manage Porter Profile and Route Qualification`:
...
- The approved PB V3.1 acceptance behavior for `Manage Porter Profile and Route Qualification` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Porter Profile and Route Qualification`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-043-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-043-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Porter Profile and Route Qualification` if they are not already implemented?

Affected:
- Jira Story: `CTMS-043`
- Logic Subtask: `CTMS-043-T01`
- UI Subtask: `CTMS-043-T02`
...
- Jira Logic Subtask `CTMS-043-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-043-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Porter Profile and Route Qualification`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-006, CTMS-013`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Story-level business rules: BR-049, BR-218, BR-252, BR-253, BR-255, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-43-manage-porter-profile-and-route-qualification.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-43-manage-porter-profile-and-route-qualification.md#ui-and-tests`
