...
Implement `View Risk Level before Trekking Registration` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Risk Level before Trekking Registration`
- Story: As a System, I want to view risk level before trekking registration so that CTMS supports the workflow safely and consistently.
...
- Dependencies are satisfied: CTMS-025, CTMS-016.
...
The system implements `View Risk Level before Trekking Registration` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Risk Level before Trekking Registration` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-026` is the direct scope and acceptance source.
...
### Scenario: View Risk Level before Trekking Registration

1. Actor opens or triggers the `View Risk Level before Trekking Registration` workflow.
...
If dependency data from `CTMS-025, CTMS-016` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Risk Level before Trekking Registration`:
...
- The approved PB V3.1 acceptance behavior for `View Risk Level before Trekking Registration` is implemented as explicit system behavior..
...
- The actor or system attempts `View Risk Level before Trekking Registration`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-026-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-026-T02`; backend/server responses remain the source of truth for server-owned business state.
...
- Jira Logic Subtask `CTMS-026-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-026-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Risk Level before Trekking Registration`
- Priority: `Must Have`
- Story points: `3.0`
- Dependencies: `CTMS-025, CTMS-016`
- Status: `To Do`
- Sprint: `Sprint 3`
