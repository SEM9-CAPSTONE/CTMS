...
**Spec Reference**  
/file/spec/ctms-15-retrieve-weather-data-for-trekking-area.md
...
Implement `Retrieve Weather Data for Trekking Area` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Retrieve Weather Data for Trekking Area`
- Story: As a System, I want to retrieve weather data for trekking area so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Retrieve Weather Data for Trekking Area` within ``.
...
The system implements `Retrieve Weather Data for Trekking Area` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Retrieve Weather Data for Trekking Area` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-039` | Required behavior for `Retrieve Weather Data for Trekking Area` must validate and enforce weather_snapshots, source, location, rainfall, wind_speed, temperature, humidity, visibility, uv_index, storm_alert as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-040` | Required behavior for `Retrieve Weather Data for Trekking Area` must validate and enforce weather, provider, timeout, critical, stale, policy, assessment as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-015` is the direct scope and acceptance source.
...
### Scenario: Retrieve Weather Data for Trekking Area

1. Actor opens or triggers the `Retrieve Weather Data for Trekking Area` workflow.
...
The implementation must persist or return only data required for `Retrieve Weather Data for Trekking Area`:
...
- The approved PB V3.1 acceptance behavior for `Retrieve Weather Data for Trekking Area` is implemented as explicit system behavior..
...
- The actor or system attempts `Retrieve Weather Data for Trekking Area`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-015-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-015-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Retrieve Weather Data for Trekking Area` if they are not already implemented?

Affected:
- Jira Story: `CTMS-015`
- Logic Subtask: `CTMS-015-T01`
- UI Subtask: `CTMS-015-T02`
...
- Jira Logic Subtask `CTMS-015-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-015-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Retrieve Weather Data for Trekking Area`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-15-retrieve-weather-data-for-trekking-area.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-15-retrieve-weather-data-for-trekking-area.md#ui-and-tests`
