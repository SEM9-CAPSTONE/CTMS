...
Implement `Calculate Multi-Criteria Weather Risk Score` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Calculate Multi-Criteria Weather Risk Score`
- Story: As a System, I want to calculate multi-criteria weather risk score so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Calculate Multi-Criteria Weather Risk Score` within `EPIC 3. Weather Risk Assessment`.
...
- Dependencies are satisfied: CTMS-015.
...
The system implements `Calculate Multi-Criteria Weather Risk Score` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Calculate Multi-Criteria Weather Risk Score` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-041` | Required behavior for `Calculate Multi-Criteria Weather Risk Score` must validate and enforce Weather, Risk, score, deterministically, weather, snapshot, weather_rules, active, backend as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-042` | Required behavior for `Calculate Multi-Criteria Weather Risk Score` must validate and enforce Weather, Risk, level, Green, Yellow, thresholds, rule, boundary, test as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-043` | Required behavior for `Calculate Multi-Criteria Weather Risk Score` must validate and enforce weather_risk_assessment, snapshot_id, rule_id, score, level, reasons, created_at, input, rule, version as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-016` is the direct scope and acceptance source.
...
### Scenario: Calculate Multi-Criteria Weather Risk Score

1. Actor opens or triggers the `Calculate Multi-Criteria Weather Risk Score` workflow.
...
If dependency data from `CTMS-015` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Calculate Multi-Criteria Weather Risk Score`:
...
- The approved PB V3.1 acceptance behavior for `Calculate Multi-Criteria Weather Risk Score` is implemented as explicit system behavior..
...
- The actor or system attempts `Calculate Multi-Criteria Weather Risk Score`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-016-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-016-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Calculate Multi-Criteria Weather Risk Score` if they are not already implemented?

Affected:
- Jira Story: `CTMS-016`
- Logic Subtask: `CTMS-016-T01`
- UI Subtask: `CTMS-016-T02`
...
- Jira Logic Subtask `CTMS-016-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-016-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Calculate Multi-Criteria Weather Risk Score`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-015`
- Status: `To Do`
- Sprint: `Sprint 2`
- Commitment: `Committed`
- Planned window: `2026-08-09` to `2026-08-22`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-16-calculate-multi-criteria-weather-risk-score.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-16-calculate-multi-criteria-weather-risk-score.md#ui-and-tests`
