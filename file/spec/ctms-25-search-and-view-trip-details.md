...
**Spec Reference**  
/file/spec/ctms-25-search-and-view-trip-details.md
...
Implement `Search and View Trip Details` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Search and View Trip Details`
- Story: As a System, I want to search and view trip details so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Search and View Trip Details` within `EPIC 4. Trip Management`.
...
- Dependencies are satisfied: CTMS-023, CTMS-016.
...
The system implements `Search and View Trip Details` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Search and View Trip Details` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-073` | Required behavior for `Search and View Trip Details` must validate and enforce Camper, discovery, Trip, centered, status, published, cancelled, search, view, khai as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-074` | Required behavior for `Search and View Trip Details` must validate and enforce Search, Trip, filter, time, range, trip_type, difficulty, price, province, city as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-075` | Required behavior for `Search and View Trip Details` must validate and enforce Trip, detail, Camper, starts_at, ends_at, meeting_point, province, city, itinerary, waypoints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-214` | Required behavior for `Search and View Trip Details` must validate and enforce API, Route, checkpoint, hazard, area, geometry, Host, Admin, System, Camper as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-215` | Required behavior for `Search and View Trip Details` must validate and enforce booking.trip_id, Camper, booking, trip_id, route_id as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-025` is the direct scope and acceptance source.
...
### Scenario: Search and View Trip Details

1. Actor opens or triggers the `Search and View Trip Details` workflow.
...
If dependency data from `CTMS-023, CTMS-016` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Search and View Trip Details`:
...
- The approved PB V3.1 acceptance behavior for `Search and View Trip Details` is implemented as explicit system behavior..
...
- The actor or system attempts `Search and View Trip Details`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-025-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-025-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Search and View Trip Details` if they are not already implemented?

Affected:
- Jira Story: `CTMS-025`
- Logic Subtask: `CTMS-025-T01`
- UI Subtask: `CTMS-025-T02`
...
- Jira Logic Subtask `CTMS-025-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-025-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Search and View Trip Details`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-023, CTMS-016`
- Status: `To Do`
- Sprint: `Sprint 3`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-25-search-and-view-trip-details.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-25-search-and-view-trip-details.md#ui-and-tests`
