...
**Spec Reference**  
/file/spec/ctms-20-configure-weather-risk-rules.md
...
Implement `Configure Weather Risk Rules` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Configure Weather Risk Rules`
- Story: As a System, I want to configure weather risk rules so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Configure Weather Risk Rules` within `EPIC 3. Weather Risk Assessment`.
...
- Dependencies are satisfied: CTMS-006, CTMS-015.
...
The system implements `Configure Weather Risk Rules` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Configure Weather Risk Rules` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-051` | Required behavior for `Configure Weather Risk Rules` must validate and enforce weather_rules, name, weights, thresholds, version, is_active, metadata, audit, unique as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-052` | Required behavior for `Configure Weather Risk Rules` must validate and enforce MVP, weather_rules, record, active, activate, rule, transaction as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-053` | Required behavior for `Configure Weather Risk Rules` must validate and enforce activate, Weather, Risk, rule, audit, actor, version, before, after, reason as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-020` is the direct scope and acceptance source.
...
### Scenario: Configure Weather Risk Rules

1. Actor opens or triggers the `Configure Weather Risk Rules` workflow.
...
If dependency data from `CTMS-006, CTMS-015` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Configure Weather Risk Rules`:
...
- The approved PB V3.1 acceptance behavior for `Configure Weather Risk Rules` is implemented as explicit system behavior..
...
- The actor or system attempts `Configure Weather Risk Rules`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-020-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-020-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Configure Weather Risk Rules` if they are not already implemented?

Affected:
- Jira Story: `CTMS-020`
- Logic Subtask: `CTMS-020-T01`
- UI Subtask: `CTMS-020-T02`
...
- Jira Logic Subtask `CTMS-020-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-020-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Configure Weather Risk Rules`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-006, CTMS-015`
- Status: `To Do`
- Sprint: `Sprint 2`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-20-configure-weather-risk-rules.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-20-configure-weather-risk-rules.md#ui-and-tests`
