...
**Spec Reference**  
/file/spec/ctms-67-view-device-data-synchronization-status.md
...
Implement `View Device Data Synchronization Status` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Device Data Synchronization Status`
- Story: As a System, I want to view device data synchronization status so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Device Data Synchronization Status` within `EPIC 10. Buffer and Synchronization`.
...
The system implements `View Device Data Synchronization Status` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Device Data Synchronization Status` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-226` | Required behavior for `View Device Data Synchronization Status` must validate and enforce offline, local, pending, data, server, confirmed, synchronized, sync, authoritative, state as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-247` | Every GPS log and safety event must have a client-generated UUID. Server upload handling must be idempotent for repeated UUIDs and must not create duplicate records or events. |
| `BR-366` | Required behavior for `View Device Data Synchronization Status` must validate and enforce package, version, integrity, audit, troubleshooting, local, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-067` is the direct scope and acceptance source.
...
### Scenario: View Device Data Synchronization Status

1. Actor opens or triggers the `View Device Data Synchronization Status` workflow.
...
The implementation must persist or return only data required for `View Device Data Synchronization Status`:
...
- The approved PB V3.1 acceptance behavior for `View Device Data Synchronization Status` is implemented as explicit system behavior..
...
- The actor or system attempts `View Device Data Synchronization Status`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-070-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-070-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Device Data Synchronization Status` if they are not already implemented?

Affected:
- Jira Story: `CTMS-067`
- Logic Subtask: `CTMS-067-T01`
- UI Subtask: `CTMS-067-T02`
...
- Jira Logic Subtask `CTMS-070-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-070-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Device Data Synchronization Status`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-70-view-device-data-synchronization-status.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-70-view-device-data-synchronization-status.md#ui-and-tests`
