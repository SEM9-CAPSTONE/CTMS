...
**Spec Reference**  
/file/spec/ctms-111-hold-successful-booking-funds.md
...
Implement `Hold Successful Booking Funds` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Hold Successful Booking Funds`
- Story: As a System, I want to hold successful booking funds so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Hold Successful Booking Funds` within `EPIC 5. Booking and Payment`.
...
- Platform: primary actor for this workflow.
...
- Dependencies are satisfied: CTMS-032.
...
The system implements `Hold Successful Booking Funds` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Hold Successful Booking Funds` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-330` | Required behavior for `Hold Successful Booking Funds` must validate and enforce charge, Booking, succeeded, backend, provider, Host, Held, Funds, held_at, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-331` | Required behavior for `Hold Successful Booking Funds` must validate and enforce Held, Funds, trace, booking, trip, host, original, successful, charge, Booking as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-332` | Required behavior for `Hold Successful Booking Funds` must validate and enforce Charge, refund, settlement, payout, adjustment, callback, retry, idempotent, business, provider as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-111` is the direct scope and acceptance source.
...
### Scenario: Hold Successful Booking Funds

1. Actor opens or triggers the `Hold Successful Booking Funds` workflow.
...
If dependency data from `CTMS-032, CTMS-035, CTMS-119, CTMS-120` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Hold Successful Booking Funds`:
...
- The approved PB V3.1 acceptance behavior for `Hold Successful Booking Funds` is implemented as explicit system behavior..
...
- The actor or system attempts `Hold Successful Booking Funds`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-117-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-117-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Hold Successful Booking Funds` if they are not already implemented?

Affected:
- Jira Story: `CTMS-111`
- Logic Subtask: `CTMS-111-T01`
- UI Subtask: `CTMS-111-T02`
...
- Jira Logic Subtask `CTMS-117-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-117-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Hold Successful Booking Funds`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-032`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-117-hold-successful-booking-funds.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-117-hold-successful-booking-funds.md#ui-and-tests`
