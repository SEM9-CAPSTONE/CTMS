...
Implement `Log Out from Current Device or All Devices` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Log Out from Current Device or All Devices`
- Story: As a System, I want to log out from current device or all devices so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Log Out from Current Device or All Devices` within `EPIC 1. Authentication`.
...
The system implements `Log Out from Current Device or All Devices` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Log Out from Current Device or All Devices` exactly as approved in the source backlog.
...
| `BR-170` | Required behavior for `Log Out from Current Device or All Devices` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-171` | Required behavior for `Log Out from Current Device or All Devices` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-008` is the direct scope and acceptance source.
...
### Scenario: Log Out from Current Device or All Devices

1. Actor opens or triggers the `Log Out from Current Device or All Devices` workflow.
...
The implementation must persist or return only data required for `Log Out from Current Device or All Devices`:
...
- The approved PB V3.1 acceptance behavior for `Log Out from Current Device or All Devices` is implemented as explicit system behavior..
...
- The actor or system attempts `Log Out from Current Device or All Devices`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-008-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-008-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Log Out from Current Device or All Devices` if they are not already implemented?

Affected:
- Jira Story: `CTMS-008`
- Logic Subtask: `CTMS-008-T01`
- UI Subtask: `CTMS-008-T02`
...
- Jira Logic Subtask `CTMS-008-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-008-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Log Out from Current Device or All Devices`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-08-log-out-from-current-device-or-all-devices.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-08-log-out-from-current-device-or-all-devices.md#ui-and-tests`
