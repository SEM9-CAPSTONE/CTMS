...
**Spec Reference**  
/file/spec/ctms-108-view-host-and-porter-reputation.md
...
Implement `View Host and Porter Reputation` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Host and Porter Reputation`
- Story: As a System, I want to view host and porter reputation so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Host and Porter Reputation` within `EPIC 16. Reviews and Feedback`.
...
- Dependencies are satisfied: CTMS-092, CTMS-093.
...
The system implements `View Host and Porter Reputation` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Host and Porter Reputation` exactly as approved in the source backlog.
...
| `BR-307` | Required behavior for `View Host and Porter Reputation` must validate and enforce API, Review, reputation, expose, khai, booking, member, data, internal, Porter as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-308` | Required behavior for `View Host and Porter Reputation` must validate and enforce Trip, Host, Porter, reputation, create, update, moderation, review, double, count as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-108` is the direct scope and acceptance source.
...
### Scenario: View Host and Porter Reputation

1. Actor opens or triggers the `View Host and Porter Reputation` workflow.
...
If dependency data from `CTMS-098, CTMS-099` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Host and Porter Reputation`:
...
- The approved PB V3.1 acceptance behavior for `View Host and Porter Reputation` is implemented as explicit system behavior..
...
- The actor or system attempts `View Host and Porter Reputation`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-114-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-114-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Host and Porter Reputation` if they are not already implemented?

Affected:
- Jira Story: `CTMS-108`
- Logic Subtask: `CTMS-108-T01`
- UI Subtask: `CTMS-108-T02`
...
- Jira Logic Subtask `CTMS-114-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-114-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Host and Porter Reputation`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-092, CTMS-093`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-114-view-host-and-porter-reputation.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-114-view-host-and-porter-reputation.md#ui-and-tests`
