...
**Spec Reference**  
/file/spec/ctms-48-view-assigned-trips.md
...
Implement `View Assigned Trips` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Assigned Trips`
- Story: As a System, I want to view assigned trips so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Assigned Trips` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-047.
...
The system implements `View Assigned Trips` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Assigned Trips` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-160` | Required behavior for `View Assigned Trips` must validate and enforce Porter, Trip, assignment, route, checkpoints, hazard, joined, members, weather, offline as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-220` | Required behavior for `View Assigned Trips` must validate and enforce Porter, eligibility, availability, qualification, assignment, conflicts as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-048` is the direct scope and acceptance source.
...
### Scenario: View Assigned Trips

1. Actor opens or triggers the `View Assigned Trips` workflow.
...
If dependency data from `CTMS-047` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Assigned Trips`:
...
- The approved PB V3.1 acceptance behavior for `View Assigned Trips` is implemented as explicit system behavior..
...
- The actor or system attempts `View Assigned Trips`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-048-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-048-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Assigned Trips` if they are not already implemented?

Affected:
- Jira Story: `CTMS-048`
- Logic Subtask: `CTMS-048-T01`
- UI Subtask: `CTMS-048-T02`
...
- Jira Logic Subtask `CTMS-048-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-048-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Assigned Trips`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-047`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-48-view-assigned-trips.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-48-view-assigned-trips.md#ui-and-tests`
