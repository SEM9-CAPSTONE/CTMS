...
**Spec Reference**  
/file/spec/ctms-114-reconcile-financial-exceptions-after-settlement-or-payout.md
...
Implement `Reconcile Financial Exceptions after Settlement or Payout` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Reconcile Financial Exceptions after Settlement or Payout`
- Story: As a System, I want to reconcile financial exceptions after settlement or payout so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Reconcile Financial Exceptions after Settlement or Payout` within `EPIC 5. Booking and Payment`.
...
- Dependencies are satisfied: CTMS-006, CTMS-112, CTMS-113.
...
The system implements `Reconcile Financial Exceptions after Settlement or Payout` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Reconcile Financial Exceptions after Settlement or Payout` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-345` | Required behavior for `Reconcile Financial Exceptions after Settlement or Payout` must validate and enforce successful, settlement, payout, ordinary, Camper, refund, request, window, Provider, chargeback as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-114` is the direct scope and acceptance source.
...
### Scenario: Reconcile Financial Exceptions after Settlement or Payout

1. Actor opens or triggers the `Reconcile Financial Exceptions after Settlement or Payout` workflow.
...
If dependency data from `CTMS-119, CTMS-006` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Reconcile Financial Exceptions after Settlement or Payout`:
...
- The approved PB V3.1 acceptance behavior for `Reconcile Financial Exceptions after Settlement or Payout` is implemented as explicit system behavior..
...
- The actor or system attempts `Reconcile Financial Exceptions after Settlement or Payout`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-120-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-120-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Reconcile Financial Exceptions after Settlement or Payout` if they are not already implemented?

Affected:
- Jira Story: `CTMS-114`
- Logic Subtask: `CTMS-114-T01`
- UI Subtask: `CTMS-114-T02`
...
- Jira Logic Subtask `CTMS-120-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-120-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Reconcile Financial Exceptions after Settlement or Payout`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-006, CTMS-112, CTMS-113`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-120-reconcile-refund-after-settlement-or-payout.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-120-reconcile-refund-after-settlement-or-payout.md#ui-and-tests`
