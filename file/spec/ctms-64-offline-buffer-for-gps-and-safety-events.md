...
**Spec Reference**  
/file/spec/ctms-64-offline-buffer-for-gps-and-safety-events.md
...
Implement `Offline Buffer for GPS and Safety Events` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Offline Buffer for GPS and Safety Events`
- Story: As a System, I want to offline buffer for gps and safety events so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Offline Buffer for GPS and Safety Events` within `EPIC 10. Buffer and Synchronization`.
...
- Dependencies are satisfied: CTMS-059.
...
The system implements `Offline Buffer for GPS and Safety Events` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Offline Buffer for GPS and Safety Events` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-242` | Required behavior for `Offline Buffer for GPS and Safety Events` must validate and enforce GPS, OFF_ROUTE, logs, safety, events, local, database, checkpoint, Offline, Safety as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-399` | Required behavior for `Offline Buffer for GPS and Safety Events` must validate and enforce GPS, Local, safety, storage, dung, cleanup, policy, record, sync, retention as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-409` | Required behavior for `Offline Buffer for GPS and Safety Events` must validate and enforce gps_logs, server, server_received_at, cleanup, retention, exception, hold, Derived, safety, events as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-064` is the direct scope and acceptance source.
...
### Scenario: Offline Buffer for GPS and Safety Events

1. Actor opens or triggers the `Offline Buffer for GPS and Safety Events` workflow.
...
If dependency data from `CTMS-059` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Offline Buffer for GPS and Safety Events`:
...
- The approved PB V3.1 acceptance behavior for `Offline Buffer for GPS and Safety Events` is implemented as explicit system behavior..
...
- The actor or system attempts `Offline Buffer for GPS and Safety Events`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-065-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-065-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Offline Buffer for GPS and Safety Events` if they are not already implemented?

Affected:
- Jira Story: `CTMS-064`
- Logic Subtask: `CTMS-064-T01`
- UI Subtask: `CTMS-064-T02`
...
- Jira Logic Subtask `CTMS-065-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-065-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Offline Buffer for GPS and Safety Events`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-059`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-65-store-gps-logs-locally-while-offline.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-65-store-gps-logs-locally-while-offline.md#ui-and-tests`
