...
**Spec Reference**  
/file/spec/ctms-84-view-active-trekking-trips-on-map.md
...
Implement `View Active Trekking Trips on Map` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Active Trekking Trips on Map`
- Story: As a System, I want to view active trekking trips on map so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Active Trekking Trips on Map` within `EPIC 14. Host Monitoring Dashboard`.
...
- Dependencies are satisfied: CTMS-010, CTMS-011, CTMS-066.
...
The system implements `View Active Trekking Trips on Map` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Active Trekking Trips on Map` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-266` | Required behavior for `View Active Trekking Trips on Map` must validate and enforce Host, active, Trips, route, checkpoint, context, members, assigned, Porters, sync as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-267` | Required behavior for `View Active Trekking Trips on Map` must validate and enforce Location, data, timestamp, state, stale, offline, real, time as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-084` is the direct scope and acceptance source.
...
### Scenario: View Active Trekking Trips on Map

1. Actor opens or triggers the `View Active Trekking Trips on Map` workflow.
...
If dependency data from `CTMS-010, CTMS-011, CTMS-069` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Active Trekking Trips on Map`:
...
- The approved PB V3.1 acceptance behavior for `View Active Trekking Trips on Map` is implemented as explicit system behavior..
...
- The actor or system attempts `View Active Trekking Trips on Map`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-089-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-089-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Active Trekking Trips on Map` if they are not already implemented?

Affected:
- Jira Story: `CTMS-084`
- Logic Subtask: `CTMS-084-T01`
- UI Subtask: `CTMS-084-T02`
...
- Jira Logic Subtask `CTMS-089-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-089-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Active Trekking Trips on Map`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-010, CTMS-011, CTMS-066`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-89-view-active-trekking-trips-on-map.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-89-view-active-trekking-trips-on-map.md#ui-and-tests`
