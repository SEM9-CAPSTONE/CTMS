...
Implement `Manage Role-Based Access for Camper, Host, Porter, and Admin` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Role-Based Access for Camper, Host, Porter, and Admin`
- Story: As a System, I want to manage role-based access for camper, host, porter, and admin so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Role-Based Access for Camper, Host, Porter, and Admin` within `EPIC 1. Authentication`.
...
- Dependencies are satisfied: CTMS-003.
...
The system implements `Manage Role-Based Access for Camper, Host, Porter, and Admin` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Role-Based Access for Camper, Host, Porter, and Admin` exactly as approved in the source backlog.
...
| `BR-170` | Required behavior for `Manage Role-Based Access for Camper, Host, Porter, and Admin` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-171` | Required behavior for `Manage Role-Based Access for Camper, Host, Porter, and Admin` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-006` is the direct scope and acceptance source.
...
### Scenario: Manage Role-Based Access for Camper, Host, Porter, and Admin

1. Actor opens or triggers the `Manage Role-Based Access for Camper, Host, Porter, and Admin` workflow.
...
If dependency data from `CTMS-003` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Manage Role-Based Access for Camper, Host, Porter, and Admin`:
...
- The approved PB V3.1 acceptance behavior for `Manage Role-Based Access for Camper, Host, Porter, and Admin` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Role-Based Access for Camper, Host, Porter, and Admin`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-006-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-006-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Role-Based Access for Camper, Host, Porter, and Admin` if they are not already implemented?

Affected:
- Jira Story: `CTMS-006`
- Logic Subtask: `CTMS-006-T01`
- UI Subtask: `CTMS-006-T02`
...
- Jira Logic Subtask `CTMS-006-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-006-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Role-Based Access for Camper, Host, Porter, and Admin`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-06-manage-role-based-access-for-camper-host-porter-and-admin.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-06-manage-role-based-access-for-camper-host-porter-and-admin.md#ui-and-tests`
