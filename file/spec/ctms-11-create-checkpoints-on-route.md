...
Implement `Create Checkpoints on Route` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Checkpoints on Route`
- Story: As a System, I want to create checkpoints on route so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Checkpoints on Route` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
The system implements `Create Checkpoints on Route` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Checkpoints on Route` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-028` | Required behavior for `Create Checkpoints on Route` must validate and enforce CHECKPOINT_REACHED, Checkpoint, Route, location, Point, type, checkpoint_type, expected_arrival_offset, instruction, guidance as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-011` is the direct scope and acceptance source.
...
### Scenario: Create Checkpoints on Route

1. Actor opens or triggers the `Create Checkpoints on Route` workflow.
...
The implementation must persist or return only data required for `Create Checkpoints on Route`:
...
- The approved PB V3.1 acceptance behavior for `Create Checkpoints on Route` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Checkpoints on Route`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-011-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-011-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create Checkpoints on Route` if they are not already implemented?

Affected:
- Jira Story: `CTMS-011`
- Logic Subtask: `CTMS-011-T01`
- UI Subtask: `CTMS-011-T02`
...
- Jira Logic Subtask `CTMS-011-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-011-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Checkpoints on Route`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-11-create-checkpoints-on-route.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-11-create-checkpoints-on-route.md#ui-and-tests`
