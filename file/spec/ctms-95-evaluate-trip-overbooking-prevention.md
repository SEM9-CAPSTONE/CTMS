...
**Spec Reference**  
/file/spec/ctms-95-evaluate-trip-overbooking-prevention.md
...
Implement `Evaluate Trip Overbooking Prevention` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Evaluate Trip Overbooking Prevention`
- Story: As a System, I want to evaluate trip overbooking prevention so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Evaluate Trip Overbooking Prevention` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-024.
...
The system implements `Evaluate Trip Overbooking Prevention` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Evaluate Trip Overbooking Prevention` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-276` | Required behavior for `Evaluate Trip Overbooking Prevention` must validate and enforce Overbooking, evaluation, concurrent, booking, scenarios, minh, release, gate, seats_taken, capacity_max as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-095` is the direct scope and acceptance source.
...
### Scenario: Evaluate Trip Overbooking Prevention

1. Actor opens or triggers the `Evaluate Trip Overbooking Prevention` workflow.
...
If dependency data from `CTMS-024` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Evaluate Trip Overbooking Prevention`:
...
- The approved PB V3.1 acceptance behavior for `Evaluate Trip Overbooking Prevention` is implemented as explicit system behavior..
...
- The actor or system attempts `Evaluate Trip Overbooking Prevention`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-101-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-101-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Evaluate Trip Overbooking Prevention` if they are not already implemented?

Affected:
- Jira Story: `CTMS-095`
- Logic Subtask: `CTMS-095-T01`
- UI Subtask: `CTMS-095-T02`
...
- Jira Logic Subtask `CTMS-101-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-101-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Evaluate Trip Overbooking Prevention`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-024`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-101-evaluate-trip-overbooking-prevention.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-101-evaluate-trip-overbooking-prevention.md#ui-and-tests`
