...
**Spec Reference**  
/file/spec/ctms-83-update-trip-availability-in-real-time.md
...
Implement `Update Trip Availability in Real Time` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Update Trip Availability in Real Time`
- Story: As a System, I want to update trip availability in real time so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Update Trip Availability in Real Time` within `EPIC 13. Real-Time Communication`.
...
- Dependencies are satisfied: CTMS-029, CTMS-024, CTMS-078.
...
The system implements `Update Trip Availability in Real Time` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Update Trip Availability in Real Time` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-264` | Required behavior for `Update Trip Availability in Real Time` must validate and enforce booking, state, capacity, client, seats_taken, remaining, seats, real, time as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-083` is the direct scope and acceptance source.
...
### Scenario: Update Trip Availability in Real Time

1. Actor opens or triggers the `Update Trip Availability in Real Time` workflow.
...
If dependency data from `CTMS-029, CTMS-024, CTMS-082` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Update Trip Availability in Real Time`:
...
- The approved PB V3.1 acceptance behavior for `Update Trip Availability in Real Time` is implemented as explicit system behavior..
...
- The actor or system attempts `Update Trip Availability in Real Time`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-087-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-087-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Update Trip Availability in Real Time` if they are not already implemented?

Affected:
- Jira Story: `CTMS-083`
- Logic Subtask: `CTMS-083-T01`
- UI Subtask: `CTMS-083-T02`
...
- Jira Logic Subtask `CTMS-087-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-087-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Update Trip Availability in Real Time`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-029, CTMS-024, CTMS-078`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-083, BR-105, BR-106, BR-107, BR-108, BR-194, BR-049, BR-218, BR-252, BR-253, BR-255, BR-270, BR-271, BR-273, BR-274
- Jira execution tasks should reference:
  - `/file/spec/ctms-87-update-trip-availability-in-real-time.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-87-update-trip-availability-in-real-time.md#ui-and-tests`
