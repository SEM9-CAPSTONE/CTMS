...
**Spec Reference**  
/file/spec/ctms-28-manage-trip-images.md
...
Implement `Manage Trip Images` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Trip Images`
- Story: As a System, I want to manage trip images so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Trip Images` within `EPIC 4. Trip Management`.
...
The system implements `Manage Trip Images` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Trip Images` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-079` | Required behavior for `Manage Trip Images` must validate and enforce Host, Trip, trip_media, ongoing, completed, sort_order as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-080` | Required behavior for `Manage Trip Images` must validate and enforce URL, trip_media, trip_id, upload, minh, type, sort_order as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-081` | Required behavior for `Manage Trip Images` must validate and enforce trip_media, Trip, status, ongoing, completed, cancelled, flow, spec as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-203` | Required behavior for `Manage Trip Images` must validate and enforce URL, Media, metadata, client, upload, minh as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-204` | Required behavior for `Manage Trip Images` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-028` is the direct scope and acceptance source.
...
### Scenario: Manage Trip Images

1. Actor opens or triggers the `Manage Trip Images` workflow.
...
The implementation must persist or return only data required for `Manage Trip Images`:
...
- The approved PB V3.1 acceptance behavior for `Manage Trip Images` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Trip Images`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-028-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-028-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Trip Images` if they are not already implemented?

Affected:
- Jira Story: `CTMS-028`
- Logic Subtask: `CTMS-028-T01`
- UI Subtask: `CTMS-028-T02`
...
- Jira Logic Subtask `CTMS-028-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-028-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Trip Images`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-28-manage-trip-images.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-28-manage-trip-images.md#ui-and-tests`
