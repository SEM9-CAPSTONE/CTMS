...
**Spec Reference**  
/file/spec/ctms-98-evaluate-ai-survival-assistant.md
...
Implement `Evaluate AI Survival Assistant` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Evaluate AI Survival Assistant`
- Story: As a System, I want to evaluate ai survival assistant so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Evaluate AI Survival Assistant` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-069, CTMS-068, CTMS-070.
...
The system implements `Evaluate AI Survival Assistant` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Evaluate AI Survival Assistant` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-279` | Required behavior for `Evaluate AI Survival Assistant` must validate and enforce RAG, P95, Survival, Assistant, evaluation, release, gates, grounded, relevant, answer as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-098` is the direct scope and acceptance source.
...
### Scenario: Evaluate AI Survival Assistant

1. Actor opens or triggers the `Evaluate AI Survival Assistant` workflow.
...
If dependency data from `CTMS-072, CTMS-071, CTMS-073` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Evaluate AI Survival Assistant`:
...
- The approved PB V3.1 acceptance behavior for `Evaluate AI Survival Assistant` is implemented as explicit system behavior..
...
- The actor or system attempts `Evaluate AI Survival Assistant`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-104-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-104-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Evaluate AI Survival Assistant` if they are not already implemented?

Affected:
- Jira Story: `CTMS-098`
- Logic Subtask: `CTMS-098-T01`
- UI Subtask: `CTMS-098-T02`
...
- Jira Logic Subtask `CTMS-104-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-104-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Evaluate AI Survival Assistant`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-069, CTMS-068, CTMS-070`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-231, BR-233, BR-235, BR-237, BR-365, BR-371, BR-372, BR-437, BR-438, BR-049, BR-218, BR-252, BR-253, BR-255, BR-270, BR-271, BR-273, BR-274, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-104-evaluate-ai-survival-assistant.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-104-evaluate-ai-survival-assistant.md#ui-and-tests`
