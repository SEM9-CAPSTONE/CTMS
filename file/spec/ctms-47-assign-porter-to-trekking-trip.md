...
Implement `Assign Porter to Trekking Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Assign Porter to Trekking Trip`
- Story: As a System, I want to assign porter to trekking trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Assign Porter to Trekking Trip` within `EPIC 7. Porter Management`.
...
- Dependencies are satisfied: CTMS-021, CTMS-043, CTMS-046.
...
The system implements `Assign Porter to Trekking Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Assign Porter to Trekking Trip` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-047` is the direct scope and acceptance source.
...
### Scenario: Assign Porter to Trekking Trip

1. Actor opens or triggers the `Assign Porter to Trekking Trip` workflow.
...
If dependency data from `CTMS-021, CTMS-043, CTMS-046` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Assign Porter to Trekking Trip`:
...
- The approved PB V3.1 acceptance behavior for `Assign Porter to Trekking Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Assign Porter to Trekking Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-047-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-047-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Assign Porter to Trekking Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-047`
- Logic Subtask: `CTMS-047-T01`
- UI Subtask: `CTMS-047-T02`
...
- Jira Logic Subtask `CTMS-047-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-047-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Assign Porter to Trekking Trip`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-021, CTMS-043, CTMS-046`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-47-assign-porter-to-trekking-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-47-assign-porter-to-trekking-trip.md#ui-and-tests`
