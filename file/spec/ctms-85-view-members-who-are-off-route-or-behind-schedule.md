...
**Spec Reference**  
/file/spec/ctms-85-view-members-who-are-off-route-or-behind-schedule.md
...
Implement `View Members Who Are Off-Route or Behind Schedule` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Members Who Are Off-Route or Behind Schedule`
- Story: As a System, I want to view members who are off-route or behind schedule so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Members Who Are Off-Route or Behind Schedule` within `EPIC 14. Host Monitoring Dashboard`.
...
- Dependencies are satisfied: CTMS-060, CTMS-063, CTMS-084.
...
The system implements `View Members Who Are Off-Route or Behind Schedule` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Members Who Are Off-Route or Behind Schedule` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-268` | Required behavior for `View Members Who Are Off-Route or Behind Schedule` must validate and enforce Host, members, flag, route, behind, schedule, filter, alert, severity, status as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-085` is the direct scope and acceptance source.
...
### Scenario: View Members Who Are Off-Route or Behind Schedule

1. Actor opens or triggers the `View Members Who Are Off-Route or Behind Schedule` workflow.
...
If dependency data from `CTMS-060, CTMS-064, CTMS-089` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Members Who Are Off-Route or Behind Schedule`:
...
- The approved PB V3.1 acceptance behavior for `View Members Who Are Off-Route or Behind Schedule` is implemented as explicit system behavior..
...
- The actor or system attempts `View Members Who Are Off-Route or Behind Schedule`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-091-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-091-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Members Who Are Off-Route or Behind Schedule` if they are not already implemented?

Affected:
- Jira Story: `CTMS-085`
- Logic Subtask: `CTMS-085-T01`
- UI Subtask: `CTMS-085-T02`
...
- Jira Logic Subtask `CTMS-091-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-091-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Members Who Are Off-Route or Behind Schedule`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-060, CTMS-063, CTMS-084`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-219, BR-239, BR-240, BR-245, BR-246, BR-247, BR-379, BR-383, BR-385, BR-386, BR-390, BR-391, BR-395, BR-396, BR-441
- Jira execution tasks should reference:
  - `/file/spec/ctms-91-view-members-who-are-off-route-or-behind-schedule.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-91-view-members-who-are-off-route-or-behind-schedule.md#ui-and-tests`
