...
**Spec Reference**  
/file/spec/ctms-33-automatically-expire-booking.md
...
Implement `Automatically Expire Booking` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Automatically Expire Booking`
- Story: As a System, I want to automatically expire booking so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Automatically Expire Booking` within `EPIC 5. Booking and Payment`.
...
- Dependencies are satisfied: CTMS-029, CTMS-032.
...
The system implements `Automatically Expire Booking` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Automatically Expire Booking` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-093` | Required behavior for `Automatically Expire Booking` must validate and enforce Booking, pending_payment, hold_expires_at, paid, expired, idempotent, update as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-094` | Required behavior for `Automatically Expire Booking` must validate and enforce booking, expired, seats, transaction as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-095` | Required behavior for `Automatically Expire Booking` must validate and enforce equipment_reservations, reserved, booking, expired, cancelled, inventory, picked_up, cancel, flow, expiry as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-096` | Required behavior for `Automatically Expire Booking` must validate and enforce NULL, Booking, expiry, audit, actor_id, system, action, booking_expired, target, booking as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-097` | Required behavior for `Automatically Expire Booking` must validate and enforce expiry, queue, commit, booking, retry, notification as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-033` is the direct scope and acceptance source.
...
### Scenario: Automatically Expire Booking

1. Actor opens or triggers the `Automatically Expire Booking` workflow.
...
If dependency data from `CTMS-029, CTMS-032` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Automatically Expire Booking`:
...
- The approved PB V3.1 acceptance behavior for `Automatically Expire Booking` is implemented as explicit system behavior..
...
- The actor or system attempts `Automatically Expire Booking`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-033-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-033-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Automatically Expire Booking` if they are not already implemented?

Affected:
- Jira Story: `CTMS-033`
- Logic Subtask: `CTMS-033-T01`
- UI Subtask: `CTMS-033-T02`
...
- Jira Logic Subtask `CTMS-033-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-033-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Automatically Expire Booking`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-029, CTMS-032`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-33-automatically-expire-booking.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-33-automatically-expire-booking.md#ui-and-tests`
