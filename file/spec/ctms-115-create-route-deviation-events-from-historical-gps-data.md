...
**Spec Reference**  
/file/spec/ctms-115-create-route-deviation-events-from-historical-gps-data.md
...
Implement `Create Route Deviation Events from Historical GPS Data` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Route Deviation Events from Historical GPS Data`
- Story: As a System, I want to create route deviation events from historical gps data so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Route Deviation Events from Historical GPS Data` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-056, CTMS-059, CTMS-060, CTMS-065.
...
The system implements `Create Route Deviation Events from Historical GPS Data` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Route Deviation Events from Historical GPS Data` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-352` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce GPS, historical, Trip, completed, route, package, version, context, geometry as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-353` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce RouteDeviationEvent, deviation, episode, gps_log, started_at, ended_at, route, trip, member, context as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-354` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce Historical, safety, analysis, RouteDeviationEvent, pattern, Route, deviation, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-411` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce GPS, Historical, deviation, analysis, quality, criteria, analytics, point, ground, truth as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-412` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce Historical, analysis, group, evidence, route, package, geometry, version, explicit, normalization as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-413` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce GPS, RouteDeviationEvent, evidence, truy, record, identifiers, range, safety, records as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-414` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce RouteDeviationEvent, generation, idempotent, analysis, version, input, scope, algorithm, rule, duplicate as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-415` | Required behavior for `Create Route Deviation Events from Historical GPS Data` must validate and enforce Derived, deviation, algorithm, rule, config, version, silently, overwrite, audit as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-115` is the direct scope and acceptance source.
...
### Scenario: Create Route Deviation Events from Historical GPS Data

1. Actor opens or triggers the `Create Route Deviation Events from Historical GPS Data` workflow.
...
If dependency data from `CTMS-035, CTMS-119, CTMS-120` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Create Route Deviation Events from Historical GPS Data`:
...
- The approved PB V3.1 acceptance behavior for `Create Route Deviation Events from Historical GPS Data` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Route Deviation Events from Historical GPS Data`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-121-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-121-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Create Route Deviation Events from Historical GPS Data` if they are not already implemented?

Affected:
- Jira Story: `CTMS-115`
- Logic Subtask: `CTMS-115-T01`
- UI Subtask: `CTMS-115-T02`
...
- Jira Logic Subtask `CTMS-121-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-121-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Route Deviation Events from Historical GPS Data`
- Priority: `Should Have`
- Story points: `13.0`
- Dependencies: `CTMS-056, CTMS-059, CTMS-060, CTMS-065`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-121-create-route-deviation-events-from-historical-gps-data.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-121-create-route-deviation-events-from-historical-gps-data.md#ui-and-tests`
