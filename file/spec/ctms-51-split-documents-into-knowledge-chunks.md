...
**Spec Reference**  
/file/spec/ctms-51-split-documents-into-knowledge-chunks.md
...
Implement `Split Documents into Knowledge Chunks` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Split Documents into Knowledge Chunks`
- Story: As a System, I want to split documents into knowledge chunks so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Split Documents into Knowledge Chunks` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-050.
...
The system implements `Split Documents into Knowledge Chunks` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Split Documents into Knowledge Chunks` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-228` | Required behavior for `Split Documents into Knowledge Chunks` must validate and enforce knowledge, chunk, content, chunk_index, metadata, source_version, locale, embedding, sinh, source as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-051` is the direct scope and acceptance source.
...
### Scenario: Split Documents into Knowledge Chunks

1. Actor opens or triggers the `Split Documents into Knowledge Chunks` workflow.
...
If dependency data from `CTMS-050` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Split Documents into Knowledge Chunks`:
...
- The approved PB V3.1 acceptance behavior for `Split Documents into Knowledge Chunks` is implemented as explicit system behavior..
...
- The actor or system attempts `Split Documents into Knowledge Chunks`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-051-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-051-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Split Documents into Knowledge Chunks` if they are not already implemented?

Affected:
- Jira Story: `CTMS-051`
- Logic Subtask: `CTMS-051-T01`
- UI Subtask: `CTMS-051-T02`
...
- Jira Logic Subtask `CTMS-051-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-051-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Split Documents into Knowledge Chunks`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-050`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-51-split-documents-into-knowledge-chunks.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-51-split-documents-into-knowledge-chunks.md#ui-and-tests`
