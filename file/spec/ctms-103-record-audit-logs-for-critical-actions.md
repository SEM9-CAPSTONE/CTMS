...
**Spec Reference**  
/file/spec/ctms-103-record-audit-logs-for-critical-actions.md
...
Implement `Record Audit Logs for Critical Actions` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Record Audit Logs for Critical Actions`
- Story: As a System, I want to record audit logs for critical actions so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Record Audit Logs for Critical Actions` within `EPIC 18. Administration and Audit`.
...
The system implements `Record Audit Logs for Critical Actions` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Record Audit Logs for Critical Actions` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-165` | Required behavior for `Record Audit Logs for Critical Actions` must validate and enforce NULL, critical, audit_logs, actor_id, system, action, target_type, created_at, before, after as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-166` | Required behavior for `Record Audit Logs for Critical Actions` must validate and enforce API, audit_logs, append, only, User, Admin, audit, record as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-169` | Required behavior for `Record Audit Logs for Critical Actions` must validate and enforce state, changing, critical, audit, policy, read as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-285` | Required behavior for `Record Audit Logs for Critical Actions` must validate and enforce Critical, action, actor, target, timestamp, before, after, reason, secrets, sensitive as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-103` is the direct scope and acceptance source.
...
### Scenario: Record Audit Logs for Critical Actions

1. Actor opens or triggers the `Record Audit Logs for Critical Actions` workflow.
...
The implementation must persist or return only data required for `Record Audit Logs for Critical Actions`:
...
- The approved PB V3.1 acceptance behavior for `Record Audit Logs for Critical Actions` is implemented as explicit system behavior..
...
- The actor or system attempts `Record Audit Logs for Critical Actions`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-109-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-109-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Record Audit Logs for Critical Actions` if they are not already implemented?

Affected:
- Jira Story: `CTMS-103`
- Logic Subtask: `CTMS-103-T01`
- UI Subtask: `CTMS-103-T02`
...
- Jira Logic Subtask `CTMS-109-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-109-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Record Audit Logs for Critical Actions`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-109-record-audit-logs-for-critical-actions.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-109-record-audit-logs-for-critical-actions.md#ui-and-tests`
