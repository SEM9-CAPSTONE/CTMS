...
**Spec Reference**  
/file/spec/ctms-73-review-ai-answers-reported-as-dangerous.md
...
Implement `Review AI Answers Reported as Dangerous` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Review AI Answers Reported as Dangerous`
- Story: As a System, I want to review ai answers reported as dangerous so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Review AI Answers Reported as Dangerous` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-006, CTMS-072.
...
The system implements `Review AI Answers Reported as Dangerous` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Review AI Answers Reported as Dangerous` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-253` | Offline survival guidance must search the downloaded local package using keyword or full-text retrieval and must not require an online LLM call. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-073` is the direct scope and acceptance source.
...
### Scenario: Review AI Answers Reported as Dangerous

1. Actor opens or triggers the `Review AI Answers Reported as Dangerous` workflow.
...
If dependency data from `CTMS-006, CTMS-075` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Review AI Answers Reported as Dangerous`:
...
- The approved PB V3.1 acceptance behavior for `Review AI Answers Reported as Dangerous` is implemented as explicit system behavior..
...
- The actor or system attempts `Review AI Answers Reported as Dangerous`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-076-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-076-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Review AI Answers Reported as Dangerous` if they are not already implemented?

Affected:
- Jira Story: `CTMS-073`
- Logic Subtask: `CTMS-073-T01`
- UI Subtask: `CTMS-073-T02`
...
- Jira Logic Subtask `CTMS-076-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-076-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Review AI Answers Reported as Dangerous`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-006, CTMS-072`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-76-review-ai-answers-reported-as-dangerous.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-76-review-ai-answers-reported-as-dangerous.md#ui-and-tests`
