...
**Spec Reference**  
/file/spec/ctms-117-review-potential-safety-hotspots.md
...
Implement `Review Potential Safety Hotspots` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Review Potential Safety Hotspots`
- Story: As a System, I want to review potential safety hotspots so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Review Potential Safety Hotspots` within `EPIC 18. Administration and Audit`.
...
- Dependencies are satisfied: CTMS-116.
...
The system implements `Review Potential Safety Hotspots` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Review Potential Safety Hotspots` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-357` | Required behavior for `Review Potential Safety Hotspots` must validate and enforce DETECTED, UNDER_REVIEW, CONFIRMED, DISMISSED, CONFIRM, PotentialSafetyHotspot, lifecycle, Admin, Host, review as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-424` | Required behavior for `Review Potential Safety Hotspots` must validate and enforce review, hotspot, audit, actor, action, timestamp, previous, status, reason as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-425` | Required behavior for `Review Potential Safety Hotspots` must validate and enforce RBAC, Host, review, confirm, hotspot, Route, Trip, role, policy, Admin as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-117` is the direct scope and acceptance source.
...
### Scenario: Review Potential Safety Hotspots

1. Actor opens or triggers the `Review Potential Safety Hotspots` workflow.
...
If dependency data from `CTMS-122` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Review Potential Safety Hotspots`:
...
- The approved PB V3.1 acceptance behavior for `Review Potential Safety Hotspots` is implemented as explicit system behavior..
...
- The actor or system attempts `Review Potential Safety Hotspots`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-123-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-123-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Review Potential Safety Hotspots` if they are not already implemented?

Affected:
- Jira Story: `CTMS-117`
- Logic Subtask: `CTMS-117-T01`
- UI Subtask: `CTMS-117-T02`
...
- Jira Logic Subtask `CTMS-123-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-123-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Review Potential Safety Hotspots`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-116`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-231, BR-233, BR-235, BR-237, BR-365, BR-371, BR-372, BR-437, BR-438, BR-049, BR-218, BR-252, BR-253, BR-255, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-123-review-potential-safety-hotspots.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-123-review-potential-safety-hotspots.md#ui-and-tests`
