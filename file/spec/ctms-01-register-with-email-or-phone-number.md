...
Implement `Register with Email or Phone Number` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Register with Email or Phone Number`
- Story: As a System, I want to register with email or phone number so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Register with Email or Phone Number` within `EPIC 1. Authentication`.
...
- Dependencies are satisfied: None.
...
The system implements `Register with Email or Phone Number` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Register with Email or Phone Number` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-001` is the direct scope and acceptance source.
...
### Scenario: Register with Email or Phone Number

1. Actor opens or triggers the `Register with Email or Phone Number` workflow.
...
If dependency data from `None` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Register with Email or Phone Number`:
...
- The approved PB V3.1 acceptance behavior for `Register with Email or Phone Number` is implemented as explicit system behavior..
...
- The actor or system attempts `Register with Email or Phone Number`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-001-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-001-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Register with Email or Phone Number` if they are not already implemented?

Affected:
- Jira Story: `CTMS-001`
- Logic Subtask: `CTMS-001-T01`
- UI Subtask: `CTMS-001-T02`
...
- Jira Logic Subtask `CTMS-001-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-001-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Register with Email or Phone Number`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `None`
- Status: `To Do`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-01-register-with-email-or-phone-number.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-01-register-with-email-or-phone-number.md#ui-and-tests`
