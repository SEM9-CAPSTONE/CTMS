...
**Spec Reference**  
/file/spec/ctms-17-view-weather-risk-factors.md
...
Implement `View Weather Risk Factors` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Weather Risk Factors`
- Story: As a System, I want to view weather risk factors so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Weather Risk Factors` within `EPIC 3. Weather Risk Assessment`.
...
The system implements `View Weather Risk Factors` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Weather Risk Factors` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-044` | Required behavior for `View Weather Risk Factors` must validate and enforce API, risk, factor, reason, threshold, score as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-045` | Required behavior for `View Weather Risk Factors` must validate and enforce Risk, level, reason as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-017` is the direct scope and acceptance source.
...
### Scenario: View Weather Risk Factors

1. Actor opens or triggers the `View Weather Risk Factors` workflow.
...
The implementation must persist or return only data required for `View Weather Risk Factors`:
...
- The approved PB V3.1 acceptance behavior for `View Weather Risk Factors` is implemented as explicit system behavior..
...
- The actor or system attempts `View Weather Risk Factors`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-017-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-017-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Weather Risk Factors` if they are not already implemented?

Affected:
- Jira Story: `CTMS-017`
- Logic Subtask: `CTMS-017-T01`
- UI Subtask: `CTMS-017-T02`
...
- Jira Logic Subtask `CTMS-017-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-017-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Weather Risk Factors`
- Priority: `Must Have`
- Story points: `3.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-17-view-weather-risk-factors.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-17-view-weather-risk-factors.md#ui-and-tests`
