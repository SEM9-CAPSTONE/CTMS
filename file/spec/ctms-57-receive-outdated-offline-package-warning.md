...
**Spec Reference**  
/file/spec/ctms-57-receive-outdated-offline-package-warning.md
...
Implement `Receive Outdated Offline Package Warning` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Receive Outdated Offline Package Warning`
- Story: As a System, I want to receive outdated offline package warning so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Receive Outdated Offline Package Warning` within `EPIC 8. Offline Package`.
...
- Dependencies are satisfied: CTMS-052, CTMS-053.
...
The system implements `Receive Outdated Offline Package Warning` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Receive Outdated Offline Package Warning` exactly as approved in the source backlog.
...
| `BR-430` | Required behavior for `Receive Outdated Offline Package Warning` must validate and enforce Offline, Safety, Package, revoke, safety, revocation, metadata, Trip, resolve as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-057` is the direct scope and acceptance source.
...
### Scenario: Receive Outdated Offline Package Warning

1. Actor opens or triggers the `Receive Outdated Offline Package Warning` workflow.
...
If dependency data from `CTMS-052, CTMS-009` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Receive Outdated Offline Package Warning`:
...
- The approved PB V3.1 acceptance behavior for `Receive Outdated Offline Package Warning` is implemented as explicit system behavior..
...
- The actor or system attempts `Receive Outdated Offline Package Warning`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-057-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-057-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Receive Outdated Offline Package Warning` if they are not already implemented?

Affected:
- Jira Story: `CTMS-057`
- Logic Subtask: `CTMS-057-T01`
- UI Subtask: `CTMS-057-T02`
...
- Jira Logic Subtask `CTMS-057-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-057-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Receive Outdated Offline Package Warning`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-052, CTMS-053`
- Status: `To Do`
- Sprint: `Sprint 4`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-57-receive-outdated-offline-package-warning.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-57-receive-outdated-offline-package-warning.md#ui-and-tests`
