...
**Spec Reference**  
/file/spec/ctms-88-receive-in-app-notifications.md
...
Implement `Receive In-App Notifications` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Receive In-App Notifications`
- Story: As a System, I want to receive in-app notifications so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Receive In-App Notifications` within `EPIC 15. Notifications`.
...
The system implements `Receive In-App Notifications` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Receive In-App Notifications` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-271` | Host must be able to view members flagged off-route or behind schedule, filter alerts by severity/status, and open the related location on the operational map. |
...
- PB V3.1 row `CTMS-088` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Receive In-App Notifications` workflow.
...
The implementation must persist or return only data required for `Receive In-App Notifications`:
...
- The approved PB V3.1 acceptance behavior for `Receive In-App Notifications` is implemented as explicit system behavior..
...
- The actor or system attempts `Receive In-App Notifications`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-094-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-094-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Receive In-App Notifications` if they are not already implemented?

Affected:
- Jira Story: `CTMS-088`
- Logic Subtask: `CTMS-088-T01`
- UI Subtask: `CTMS-088-T02`
...
- Jira Logic Subtask `CTMS-094-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-094-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Receive In-App Notifications`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-94-receive-in-app-notifications.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-94-receive-in-app-notifications.md#ui-and-tests`
