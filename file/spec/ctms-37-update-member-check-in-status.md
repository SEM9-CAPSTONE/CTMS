...
**Spec Reference**  
/file/spec/ctms-37-update-member-check-in-status.md
...
Implement `Update Member Check-In Status` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Update Member Check-In Status`
- Story: As a System, I want to update member check-in status so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Update Member Check-In Status` within `EPIC 5. Booking and Payment`.
...
- Dependencies are satisfied: CTMS-030, CTMS-031.
...
The system implements `Update Member Check-In Status` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Update Member Check-In Status` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-110` | Required behavior for `Update Member Check-In Status` must validate and enforce booking, status, confirmed, Trip, check, policy as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-111` | Required behavior for `Update Member Check-In Status` must validate and enforce booking_members.member_status, Check, booking_members, member_status, joined, checked_in_at, status_updated_by, check as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-112` | Required behavior for `Update Member Check-In Status` must validate and enforce Check, member, booking, completed, confirmed, flow as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-113` | Required behavior for `Update Member Check-In Status` must validate and enforce check, member, status, removed, no_show, left, booking, cancelled, expired, actor as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-114` | Required behavior for `Update Member Check-In Status` must validate and enforce member, Trip, member_status, left, left_at, status_updated_by, joined, override, audit as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-118` | Required behavior for `Update Member Check-In Status` must validate and enforce check, policy, member, registered, joined, no_show, manual, action as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-119` | Required behavior for `Update Member Check-In Status` must validate and enforce NULL, show, no_show_at, status_updated_by, system, audit, policy as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-120` | Required behavior for `Update Member Check-In Status` must validate and enforce booking_status, no_show, booking_member as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-048` | Required behavior for `Update Member Check-In Status` must validate and enforce Trip, bookings, booking_members, entity, Member as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-037` is the direct scope and acceptance source.
...
### Scenario: Update Member Check-In Status

1. Actor opens or triggers the `Update Member Check-In Status` workflow.
...
If dependency data from `CTMS-030, CTMS-031` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Update Member Check-In Status`:
...
- The approved PB V3.1 acceptance behavior for `Update Member Check-In Status` is implemented as explicit system behavior..
...
- The actor or system attempts `Update Member Check-In Status`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-037-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-037-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Update Member Check-In Status` if they are not already implemented?

Affected:
- Jira Story: `CTMS-037`
- Logic Subtask: `CTMS-037-T01`
- UI Subtask: `CTMS-037-T02`
...
- Jira Logic Subtask `CTMS-037-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-037-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Update Member Check-In Status`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-030, CTMS-031`
- Status: `To Do`
- Sprint: `Sprint 3`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-37-update-member-check-in-status.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-37-update-member-check-in-status.md#ui-and-tests`
