...
Implement `Store Important Health Information` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Store Important Health Information`
- Story: As a System, I want to store important health information so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Store Important Health Information` within `EPIC 1. Authentication`.
...
- Dependencies are satisfied: CTMS-007.
...
The system implements `Store Important Health Information` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Store Important Health Information` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-025` | `user_medical_profiles` may be accessed by Host or Porter only when the user has consented and the actor belongs to an authorized related Trip; backend authorization is required on every request. |
| `BR-026` | Required behavior for `Store Important Health Information` must validate and enforce User, sharing_consent, consent, request, Host, Porter as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-187` | Required behavior for `Store Important Health Information` must validate and enforce truy, consent, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-170` | Required behavior for `Store Important Health Information` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-171` | Required behavior for `Store Important Health Information` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-009` is the direct scope and acceptance source.
...
### Scenario: Store Important Health Information

1. Actor opens or triggers the `Store Important Health Information` workflow.
...
If dependency data from `CTMS-007` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Store Important Health Information`:
...
- The approved PB V3.1 acceptance behavior for `Store Important Health Information` is implemented as explicit system behavior..
...
- The actor or system attempts `Store Important Health Information`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-009-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-009-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Store Important Health Information` if they are not already implemented?

Affected:
- Jira Story: `CTMS-009`
- Logic Subtask: `CTMS-009-T01`
- UI Subtask: `CTMS-009-T02`
...
- Jira Logic Subtask `CTMS-009-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-009-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Store Important Health Information`
...
- Jira execution tasks should reference:
  ...
  - `/file/spec/ctms-09-store-important-health-information.md#ui-and-tests`
