...
**Spec Reference**  
/file/spec/ctms-68-use-retrieval-augmented-generation-rag.md
...
Implement `Use Retrieval-Augmented Generation (RAG)` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Use Retrieval-Augmented Generation (RAG)`
- Story: As a System, I want to use retrieval-augmented generation (rag) so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Use Retrieval-Augmented Generation (RAG)` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-051.
...
The system implements `Use Retrieval-Augmented Generation (RAG)` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Use Retrieval-Augmented Generation (RAG)` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-248` | Required behavior for `Use Retrieval-Augmented Generation (RAG)` must validate and enforce RAG, retrieve, knowledge, chunks, context, limit, source, references, language as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-068` is the direct scope and acceptance source.
...
### Scenario: Use Retrieval-Augmented Generation (RAG)

1. Actor opens or triggers the `Use Retrieval-Augmented Generation (RAG)` workflow.
...
If dependency data from `CTMS-051` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Use Retrieval-Augmented Generation (RAG)`:
...
- The approved PB V3.1 acceptance behavior for `Use Retrieval-Augmented Generation (RAG)` is implemented as explicit system behavior..
...
- The actor or system attempts `Use Retrieval-Augmented Generation (RAG)`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-071-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-071-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Use Retrieval-Augmented Generation (RAG)` if they are not already implemented?

Affected:
- Jira Story: `CTMS-068`
- Logic Subtask: `CTMS-068-T01`
- UI Subtask: `CTMS-068-T02`
...
- Jira Logic Subtask `CTMS-071-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-071-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Use Retrieval-Augmented Generation (RAG)`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-051`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-71-use-retrieval-augmented-generation-rag.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-71-use-retrieval-augmented-generation-rag.md#ui-and-tests`
