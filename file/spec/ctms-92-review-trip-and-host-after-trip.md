...
**Spec Reference**  
/file/spec/ctms-92-review-trip-and-host-after-trip.md
...
Implement `Review Trip and Host after Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Review Trip and Host after Trip`
- Story: As a System, I want to review trip and host after trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Review Trip and Host after Trip` within `EPIC 16. Reviews and Feedback`.
...
- Dependencies are satisfied: CTMS-038.
...
The system implements `Review Trip and Host after Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Review Trip and Host after Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-297` | Required behavior for `Review Trip and Host after Trip` must validate and enforce booking.completed_at, Trip, review, Camper, completed, booking, participation, completed_at, trip_id, reviewer as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-298` | Required behavior for `Review Trip and Host after Trip` must validate and enforce Host, Trip, backend, host_id, trip_id, client as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-300` | Required behavior for `Review Trip and Host after Trip` must validate and enforce booking.completed_at, reviewer, review, target, Trip, participation, duplicate, submit, idempotent, Camper as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-301` | Required behavior for `Review Trip and Host after Trip` must validate and enforce Rating, point, rating, comment, validation, user, review, authoritative, input as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-092` is the direct scope and acceptance source.
...
### Scenario: Review Trip and Host after Trip

1. Actor opens or triggers the `Review Trip and Host after Trip` workflow.
...
If dependency data from `CTMS-038` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Review Trip and Host after Trip`:
...
- The approved PB V3.1 acceptance behavior for `Review Trip and Host after Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Review Trip and Host after Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-098-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-098-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Review Trip and Host after Trip` if they are not already implemented?

Affected:
- Jira Story: `CTMS-092`
- Logic Subtask: `CTMS-092-T01`
- UI Subtask: `CTMS-092-T02`
...
- Jira Logic Subtask `CTMS-098-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-098-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Review Trip and Host after Trip`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-038`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-98-review-trip-and-host-after-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-98-review-trip-and-host-after-trip.md#ui-and-tests`
