...
**Spec Reference**  
/file/spec/ctms-110-view-host-revenue-analytics-by-time-and-area.md
...
Implement `View Host Revenue Analytics by Time and Area` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Host Revenue Analytics by Time and Area`
- Story: As a System, I want to view host revenue analytics by time and area so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Host Revenue Analytics by Time and Area` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-025, CTMS-029.
...
The system implements `View Host Revenue Analytics by Time and Area` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Host Revenue Analytics by Time and Area` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-318` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Host, revenue, analytics, Trip, financial, records, business, scope, Admin as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-319` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Revenue, Analytics, Trip, province_code, city_code, snapshot, authoritative, area, dimension, Snapshot as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-320` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Gross, Revenue, successful, captured, charge, financial, transactions, booking, draft, pending as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-321` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Refund, Amount, refund, transactions, succeeded, pending, failed as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-322` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Platform, snapshot, applied, settlement, commission, rate as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-323` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Revenue, authoritative, financial, ledger, eligible, gross, succeeded, refunds, platform, approved as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-324` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce KPI, Payout, amount, Revenue, nhau, entitlement, settlement, adjustment, cash, transfer as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-325` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce Held, Pending, funds, settled, payable, paid, amounts, Funds, doanh, payout as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-326` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce KPI, trips.starts_at, Revenue, analytics, filter, time, range, Trip, province, city as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-327` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce MVP, settlement, reporting, currency, financial, scope, transaction, future, release, multi as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-328` | Actual financial revenue must come from the payment/refund/settlement/adjustment/payout ledger. Any estimated operational revenue must be labeled Estimated and kept separate from actual revenue. |
| `BR-329` | Required behavior for `View Host Revenue Analytics by Time and Area` must validate and enforce aggregate, revenue, truy, Trip, Booking, financial, transaction, audit, reconciliation as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-110` is the direct scope and acceptance source.
...
### Scenario: View Host Revenue Analytics by Time and Area

1. Actor opens or triggers the `View Host Revenue Analytics by Time and Area` workflow.
...
If dependency data from `CTMS-025, CTMS-029` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Host Revenue Analytics by Time and Area`:
...
- The approved PB V3.1 acceptance behavior for `View Host Revenue Analytics by Time and Area` is implemented as explicit system behavior..
...
- The actor or system attempts `View Host Revenue Analytics by Time and Area`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-116-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-116-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Host Revenue Analytics by Time and Area` if they are not already implemented?

Affected:
- Jira Story: `CTMS-110`
- Logic Subtask: `CTMS-110-T01`
- UI Subtask: `CTMS-110-T02`
...
- Jira Logic Subtask `CTMS-116-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-116-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Host Revenue Analytics by Time and Area`
- Priority: `Should Have`
- Story points: `13.0`
- Dependencies: `CTMS-025, CTMS-029`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-083, BR-105, BR-106, BR-107, BR-108, BR-194, BR-049, BR-218, BR-252, BR-253, BR-255, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-116-view-host-revenue-analytics-by-time-and-area.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-116-view-host-revenue-analytics-by-time-and-area.md#ui-and-tests`
