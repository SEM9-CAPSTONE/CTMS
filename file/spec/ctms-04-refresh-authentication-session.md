...
Implement `Refresh Authentication Session` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Refresh Authentication Session`
- Story: As a System, I want to refresh authentication session so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Refresh Authentication Session` within `EPIC 1. Authentication`.
...
The system implements `Refresh Authentication Session` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Refresh Authentication Session` exactly as approved in the source backlog.
...
| `BR-170` | Required behavior for `Refresh Authentication Session` must validate and enforce the story-specific validation, authorization, state, persistence, audit, notification, and testing constraints as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-171` | Required behavior for `Refresh Authentication Session` must validate and enforce active, account, backend, request, pending_verification, suspended, deleted, client, access as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-004` is the direct scope and acceptance source.
...
### Scenario: Refresh Authentication Session

1. Actor opens or triggers the `Refresh Authentication Session` workflow.
...
The implementation must persist or return only data required for `Refresh Authentication Session`:
...
- The approved PB V3.1 acceptance behavior for `Refresh Authentication Session` is implemented as explicit system behavior..
...
- The actor or system attempts `Refresh Authentication Session`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-004-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-004-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Refresh Authentication Session` if they are not already implemented?

Affected:
- Jira Story: `CTMS-004`
- Logic Subtask: `CTMS-004-T01`
- UI Subtask: `CTMS-004-T02`
...
- Jira Logic Subtask `CTMS-004-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-004-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Refresh Authentication Session`
- Priority: `Must Have`
- Story points: `3.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-04-refresh-authentication-session.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-04-refresh-authentication-session.md#ui-and-tests`
