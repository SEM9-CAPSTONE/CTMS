...
**Spec Reference**  
/file/spec/ctms-54-verify-offline-package-integrity.md
...
Implement `Verify Offline Package Integrity` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Verify Offline Package Integrity`
- Story: As a System, I want to verify offline package integrity so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Verify Offline Package Integrity` within `EPIC 8. Offline Package`.
...
- Dependencies are satisfied: CTMS-053.
...
The system implements `Verify Offline Package Integrity` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Verify Offline Package Integrity` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-232` | Required behavior for `Verify Offline Package Integrity` must validate and enforce system, checksum, package, failed, status, corrupted, enum as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-054` is the direct scope and acceptance source.
...
### Scenario: Verify Offline Package Integrity

1. Actor opens or triggers the `Verify Offline Package Integrity` workflow.
...
If dependency data from `CTMS-053` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Verify Offline Package Integrity`:
...
- The approved PB V3.1 acceptance behavior for `Verify Offline Package Integrity` is implemented as explicit system behavior..
...
- The actor or system attempts `Verify Offline Package Integrity`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-054-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-054-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Verify Offline Package Integrity` if they are not already implemented?

Affected:
- Jira Story: `CTMS-054`
- Logic Subtask: `CTMS-054-T01`
- UI Subtask: `CTMS-054-T02`
...
- Jira Logic Subtask `CTMS-054-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-054-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Verify Offline Package Integrity`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-053`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-54-verify-offline-package-integrity.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-54-verify-offline-package-integrity.md#ui-and-tests`
