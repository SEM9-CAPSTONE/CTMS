...
**Spec Reference**  
/file/spec/ctms-96-evaluate-gps-deviation-detection.md
...
Implement `Evaluate GPS Deviation Detection` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Evaluate GPS Deviation Detection`
- Story: As a System, I want to evaluate gps deviation detection so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Evaluate GPS Deviation Detection` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-059, CTMS-060, CTMS-065.
...
The system implements `Evaluate GPS Deviation Detection` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Evaluate GPS Deviation Detection` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-277` | Required behavior for `Evaluate GPS Deviation Detection` must validate and enforce GPS, P95, OFF_ROUTE, deviation, evaluation, ground, truth, dataset, release, gates as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-433` | Required behavior for `Evaluate GPS Deviation Detection` must validate and enforce Field, evaluation, threshold, state, transition, Safety, Tracking, Business, Rules, config as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-434` | Required behavior for `Evaluate GPS Deviation Detection` must validate and enforce OFF_ROUTE, P95, GPS, Field, evaluation, false, positive, rate, negative, detection as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-435` | Required behavior for `Evaluate GPS Deviation Detection` must validate and enforce P95, Field, evaluation, checkpoint, detection, missed, arrival, rate, false, latency as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-438` | Package generation must snapshot or reference exact versions of all authoritative inputs needed for audit. Storing only a generic version number that cannot identify the Route/checkpoint/hazard state included in the package is insufficient. |
| `BR-439` | Required behavior for `Evaluate GPS Deviation Detection` must validate and enforce GPS, Field, test, ground, truth, reference, position, sample as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-096` is the direct scope and acceptance source.
...
### Scenario: Evaluate GPS Deviation Detection

1. Actor opens or triggers the `Evaluate GPS Deviation Detection` workflow.
...
If dependency data from `CTMS-060` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Evaluate GPS Deviation Detection`:
...
- The approved PB V3.1 acceptance behavior for `Evaluate GPS Deviation Detection` is implemented as explicit system behavior..
...
- The actor or system attempts `Evaluate GPS Deviation Detection`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-102-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-102-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Evaluate GPS Deviation Detection` if they are not already implemented?

Affected:
- Jira Story: `CTMS-096`
- Logic Subtask: `CTMS-096-T01`
- UI Subtask: `CTMS-096-T02`
...
- Jira Logic Subtask `CTMS-102-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-102-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Evaluate GPS Deviation Detection`
- Priority: `Must Have`
- Story points: `13.0`
- Dependencies: `CTMS-059, CTMS-060, CTMS-065`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-102-evaluate-gps-deviation-detection.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-102-evaluate-gps-deviation-detection.md#ui-and-tests`
