...
**Spec Reference**  
/file/spec/ctms-61-automatically-detect-checkpoint-arrival.md
...
Implement `Automatically Detect Checkpoint Arrival` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Automatically Detect Checkpoint Arrival`
- Story: As a System, I want to automatically detect checkpoint arrival so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Automatically Detect Checkpoint Arrival` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-011, CTMS-052, CTMS-059.
...
The system implements `Automatically Detect Checkpoint Arrival` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Automatically Detect Checkpoint Arrival` exactly as approved in the source backlog.
...
| `BR-392` | Required behavior for `Automatically Detect Checkpoint Arrival` must validate and enforce CHECKPOINT_REACHED, GPS, VALID, Route, Checkpoint, samples, distance_to_checkpoint as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-393` | Required behavior for `Automatically Detect Checkpoint Arrival` must validate and enforce GPS, checkpoint, arrival, trip, member, duplicate, visit, samples, radius, idempotency as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-394` | Required behavior for `Automatically Detect Checkpoint Arrival` must validate and enforce MVP, quay, checkpoint, repeat, visit, duplicate, samples, entry, policy, server as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-440` | Required behavior for `Automatically Detect Checkpoint Arrival` must validate and enforce CHECKPOINT_REACHED, Safety, checkpoint, detection, Route, Checkpoint, Rest, Meal, Activity, custom as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-061` is the direct scope and acceptance source.
...
### Scenario: Automatically Detect Checkpoint Arrival

1. Actor opens or triggers the `Automatically Detect Checkpoint Arrival` workflow.
...
If dependency data from `CTMS-011, CTMS-059` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Automatically Detect Checkpoint Arrival`:
...
- The approved PB V3.1 acceptance behavior for `Automatically Detect Checkpoint Arrival` is implemented as explicit system behavior..
...
- The actor or system attempts `Automatically Detect Checkpoint Arrival`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-061-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-061-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Automatically Detect Checkpoint Arrival` if they are not already implemented?

Affected:
- Jira Story: `CTMS-061`
- Logic Subtask: `CTMS-061-T01`
- UI Subtask: `CTMS-061-T02`
...
- Jira Logic Subtask `CTMS-061-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-061-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Automatically Detect Checkpoint Arrival`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-011, CTMS-052, CTMS-059`
- Status: `To Do`
- Sprint: `Sprint 4`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-61-automatically-detect-checkpoint-arrival.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-61-automatically-detect-checkpoint-arrival.md#ui-and-tests`
