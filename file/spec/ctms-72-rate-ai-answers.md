...
**Spec Reference**  
/file/spec/ctms-72-rate-ai-answers.md
...
Implement `Rate AI Answers` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Rate AI Answers`
- Story: As a System, I want to rate ai answers so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Rate AI Answers` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-069.
...
The system implements `Rate AI Answers` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Rate AI Answers` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-252` | The AI assistant must answer first-aid and survival questions using the configured CTMS knowledge base within the operational latency target and according to applicable safety constraints. |
...
- PB V3.1 row `CTMS-072` is the direct scope and acceptance source.
...
### Scenario: Rate AI Answers

1. Actor opens or triggers the `Rate AI Answers` workflow.
...
If dependency data from `CTMS-072` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Rate AI Answers`:
...
- The approved PB V3.1 acceptance behavior for `Rate AI Answers` is implemented as explicit system behavior..
...
- The actor or system attempts `Rate AI Answers`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-075-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-075-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Rate AI Answers` if they are not already implemented?

Affected:
- Jira Story: `CTMS-072`
- Logic Subtask: `CTMS-072-T01`
- UI Subtask: `CTMS-072-T02`
...
- Jira Logic Subtask `CTMS-075-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-075-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Rate AI Answers`
- Priority: `Should Have`
- Story points: `3.0`
- Dependencies: `CTMS-069`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-75-rate-ai-answers.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-75-rate-ai-answers.md#ui-and-tests`
