...
**Spec Reference**  
/file/spec/ctms-13-approve-trekking-route.md
...
Implement `Approve Trekking Route` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Approve Trekking Route`
- Story: As a System, I want to approve trekking route so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Approve Trekking Route` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
- Dependencies are satisfied: CTMS-006, CTMS-010, CTMS-011.
...
The system implements `Approve Trekking Route` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Approve Trekking Route` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-031` | Required behavior for `Approve Trekking Route` must validate and enforce route.status, Admin, Route, route, status, pending_approval, geometry, difficulty, checkpoint, hazard as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-032` | Required behavior for `Approve Trekking Route` must validate and enforce Route, status, closed, actor, audit, Trip as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-033` | Required behavior for `Approve Trekking Route` must validate and enforce route, status, rejected, state, transition, enum, route_status as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-037` | Required behavior for `Approve Trekking Route` must validate and enforce Trip, submit, publish, bind, approved, Route, version, immutable, snapshot as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-013` is the direct scope and acceptance source.
...
### Scenario: Approve Trekking Route

1. Actor opens or triggers the `Approve Trekking Route` workflow.
...
If dependency data from `CTMS-006, CTMS-010, CTMS-011` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Approve Trekking Route`:
...
- The approved PB V3.1 acceptance behavior for `Approve Trekking Route` is implemented as explicit system behavior..
...
- The actor or system attempts `Approve Trekking Route`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-013-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-013-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Approve Trekking Route` if they are not already implemented?

Affected:
- Jira Story: `CTMS-013`
- Logic Subtask: `CTMS-013-T01`
- UI Subtask: `CTMS-013-T02`
...
- Jira Logic Subtask `CTMS-013-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-013-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Approve Trekking Route`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-006, CTMS-010, CTMS-011`
- Status: `To Do`
- Sprint: `Sprint 2`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-13-approve-trekking-route.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-13-approve-trekking-route.md#ui-and-tests`
