...
**Spec Reference**  
/file/spec/ctms-79-receive-real-time-sos-alerts.md
...
Implement `Receive Real-Time SOS Alerts` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Receive Real-Time SOS Alerts`
- Story: As a System, I want to receive real-time sos alerts so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Receive Real-Time SOS Alerts` within `EPIC 12. SOS and Emergency Management`.
...
- Dependencies are satisfied: CTMS-074, CTMS-078.
...
The system implements `Receive Real-Time SOS Alerts` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Receive Real-Time SOS Alerts` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-079` is the direct scope and acceptance source.
...
### Scenario: Receive Real-Time SOS Alerts

1. Actor opens or triggers the `Receive Real-Time SOS Alerts` workflow.
...
If dependency data from `CTMS-077, CTMS-082` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Receive Real-Time SOS Alerts`:
...
- The approved PB V3.1 acceptance behavior for `Receive Real-Time SOS Alerts` is implemented as explicit system behavior..
...
- The actor or system attempts `Receive Real-Time SOS Alerts`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-083-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-083-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Receive Real-Time SOS Alerts` if they are not already implemented?

Affected:
- Jira Story: `CTMS-079`
- Logic Subtask: `CTMS-079-T01`
- UI Subtask: `CTMS-079-T02`
...
- Jira Logic Subtask `CTMS-083-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-083-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Receive Real-Time SOS Alerts`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-074, CTMS-078`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-83-receive-real-time-sos-alerts.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-83-receive-real-time-sos-alerts.md#ui-and-tests`
