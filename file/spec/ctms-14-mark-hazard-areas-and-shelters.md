...
**Spec Reference**  
/file/spec/ctms-14-mark-hazard-areas-and-shelters.md
...
Implement `Mark Hazard Areas and Shelters` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Mark Hazard Areas and Shelters`
- Story: As a System, I want to mark hazard areas and shelters so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Mark Hazard Areas and Shelters` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
- Dependencies are satisfied: CTMS-010.
...
The system implements `Mark Hazard Areas and Shelters` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Mark Hazard Areas and Shelters` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-034` | Required behavior for `Mark Hazard Areas and Shelters` must validate and enforce Checkpoint, hazard, area, Host, Admin, safety, offline, features, public, Route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-035` | Required behavior for `Mark Hazard Areas and Shelters` must validate and enforce Checkpoint, hazard, data, Trip, offline, package, version, route, outdated as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-036` | Required behavior for `Mark Hazard Areas and Shelters` must validate and enforce hazard, area, Route, Polygon, description, severity, shelter, water, overnight, checkpoint as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-014` is the direct scope and acceptance source.
...
### Scenario: Mark Hazard Areas and Shelters

1. Actor opens or triggers the `Mark Hazard Areas and Shelters` workflow.
...
If dependency data from `CTMS-010` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Mark Hazard Areas and Shelters`:
...
- The approved PB V3.1 acceptance behavior for `Mark Hazard Areas and Shelters` is implemented as explicit system behavior..
...
- The actor or system attempts `Mark Hazard Areas and Shelters`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-014-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-014-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Mark Hazard Areas and Shelters` if they are not already implemented?

Affected:
- Jira Story: `CTMS-014`
- Logic Subtask: `CTMS-014-T01`
- UI Subtask: `CTMS-014-T02`
...
- Jira Logic Subtask `CTMS-014-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-014-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Mark Hazard Areas and Shelters`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-14-mark-hazard-areas-and-shelters.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-14-mark-hazard-areas-and-shelters.md#ui-and-tests`
