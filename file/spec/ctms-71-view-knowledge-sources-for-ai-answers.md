...
**Spec Reference**  
/file/spec/ctms-71-view-knowledge-sources-for-ai-answers.md
...
Implement `View Knowledge Sources for AI Answers` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `View Knowledge Sources for AI Answers`
- Story: As a System, I want to view knowledge sources for ai answers so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `View Knowledge Sources for AI Answers` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-069, CTMS-068.
...
The system implements `View Knowledge Sources for AI Answers` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `View Knowledge Sources for AI Answers` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-251` | Required behavior for `View Knowledge Sources for AI Answers` must validate and enforce answer, document, source, category, retrieval, metadata as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-212` | Any Business Rule, enum, state transition, or API contract change must update the spec, tests, and data documentation before the story is Done. |
| `BR-213` | Every mapped Business Rule must have at least one valid-path test and one violation-path test; concurrency, idempotency, and transaction rules require integration or E2E coverage. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-071` is the direct scope and acceptance source.
...
### Scenario: View Knowledge Sources for AI Answers

1. Actor opens or triggers the `View Knowledge Sources for AI Answers` workflow.
...
If dependency data from `CTMS-072, CTMS-071` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `View Knowledge Sources for AI Answers`:
...
- The approved PB V3.1 acceptance behavior for `View Knowledge Sources for AI Answers` is implemented as explicit system behavior..
...
- The actor or system attempts `View Knowledge Sources for AI Answers`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-074-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-074-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `View Knowledge Sources for AI Answers` if they are not already implemented?

Affected:
- Jira Story: `CTMS-071`
- Logic Subtask: `CTMS-071-T01`
- UI Subtask: `CTMS-071-T02`
...
- Jira Logic Subtask `CTMS-074-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-074-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `View Knowledge Sources for AI Answers`
- Priority: `Should Have`
- Story points: `3.0`
- Dependencies: `CTMS-069, CTMS-068`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-74-view-knowledge-sources-for-ai-answers.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-74-view-knowledge-sources-for-ai-answers.md#ui-and-tests`
