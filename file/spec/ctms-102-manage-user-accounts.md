...
**Spec Reference**  
/file/spec/ctms-102-manage-user-accounts.md
...
Implement `Manage User Accounts` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage User Accounts`
- Story: As a System, I want to manage user accounts so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage User Accounts` within `EPIC 18. Administration and Audit`.
...
The system implements `Manage User Accounts` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage User Accounts` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-163` | Required behavior for `Manage User Accounts` must validate and enforce Admin, search, view, suspend, reactivate, user, hard, delete as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-164` | Required behavior for `Manage User Accounts` must validate and enforce User, hard, delete, status, soft, schema, audit as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-284` | Required behavior for `Manage User Accounts` must validate and enforce Admin, search, view, lock, unlock, user, accounts, account, management, related as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-102` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Manage User Accounts` workflow.
...
The implementation must persist or return only data required for `Manage User Accounts`:
...
- The approved PB V3.1 acceptance behavior for `Manage User Accounts` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage User Accounts`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-108-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-108-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage User Accounts` if they are not already implemented?

Affected:
- Jira Story: `CTMS-102`
- Logic Subtask: `CTMS-102-T01`
- UI Subtask: `CTMS-102-T02`
...
- Jira Logic Subtask `CTMS-108-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-108-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage User Accounts`
- Priority: `Must Have`
- Story points: `5.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-108-manage-user-accounts.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-108-manage-user-accounts.md#ui-and-tests`
