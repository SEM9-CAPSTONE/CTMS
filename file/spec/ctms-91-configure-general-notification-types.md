...
**Spec Reference**  
/file/spec/ctms-91-configure-general-notification-types.md
...
Implement `Configure General Notification Types` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Configure General Notification Types`
- Story: As a System, I want to configure general notification types so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Configure General Notification Types` within `EPIC 15. Notifications`.
...
The system implements `Configure General Notification Types` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Configure General Notification Types` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-091` is the direct scope and acceptance source.
...
### Scenario: Configure General Notification Types

1. Actor opens or triggers the `Configure General Notification Types` workflow.
...
The implementation must persist or return only data required for `Configure General Notification Types`:
...
- The approved PB V3.1 acceptance behavior for `Configure General Notification Types` is implemented as explicit system behavior..
...
- The actor or system attempts `Configure General Notification Types`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-097-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-097-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Configure General Notification Types` if they are not already implemented?

Affected:
- Jira Story: `CTMS-091`
- Logic Subtask: `CTMS-091-T01`
- UI Subtask: `CTMS-091-T02`
...
- Jira Logic Subtask `CTMS-097-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-097-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Configure General Notification Types`
- Priority: `Could Have`
- Story points: `3.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-97-configure-general-notification-types.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-97-configure-general-notification-types.md#ui-and-tests`
