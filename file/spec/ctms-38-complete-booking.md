...
**Spec Reference**  
/file/spec/ctms-38-complete-booking.md
...
Implement `Complete Booking` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Complete Booking`
- Story: As a System, I want to complete booking so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Complete Booking` within `EPIC 5. Booking and Payment`.
...
- Dependencies are satisfied: CTMS-037, CTMS-056.
...
The system implements `Complete Booking` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Complete Booking` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-115` | Required behavior for `Complete Booking` must validate and enforce bookings.completed_at, Booking, confirmed, completed, Trip, status, member, equipment, condition, transaction as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-116` | Required behavior for `Complete Booking` must validate and enforce complete, booking, equipment, rental, reservation, returned, not_returned, damage, flow as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-117` | Required behavior for `Complete Booking` must validate and enforce booking, completed, review, target as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-048` | Required behavior for `Complete Booking` must validate and enforce Trip, bookings, booking_members, entity, Member as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-038` is the direct scope and acceptance source.
...
1. Actor opens or triggers the `Complete Booking` workflow.
...
If dependency data from `CTMS-037, CTMS-056` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Complete Booking`:
...
- The approved PB V3.1 acceptance behavior for `Complete Booking` is implemented as explicit system behavior..
...
- The actor or system attempts `Complete Booking`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-038-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-038-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Complete Booking` if they are not already implemented?

Affected:
- Jira Story: `CTMS-038`
- Logic Subtask: `CTMS-038-T01`
- UI Subtask: `CTMS-038-T02`
...
- Jira Logic Subtask `CTMS-038-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-038-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Complete Booking`
- Priority: `Should Have`
- Story points: `5.0`
- Dependencies: `CTMS-037, CTMS-056`
- Status: `To Do`
- Sprint: `Sprint 3`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-38-complete-booking.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-38-complete-booking.md#ui-and-tests`
