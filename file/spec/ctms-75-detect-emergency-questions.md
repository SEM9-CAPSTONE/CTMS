...
**Spec Reference**  
/file/spec/ctms-75-detect-emergency-questions.md
...
Implement `Detect Emergency Questions` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Detect Emergency Questions`
- Story: As a System, I want to detect emergency questions so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Detect Emergency Questions` within `EPIC 11. AI Survival Assistant and RAG`.
...
- Dependencies are satisfied: CTMS-069, CTMS-074.
...
The system implements `Detect Emergency Questions` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Detect Emergency Questions` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-255` | A Camper may rate an AI answer as Useful or Not Useful and may optionally report it unsafe. Feedback must store the rating, optional reason, unsafe flag, and support offline feedback sync. |
...
- PB V3.1 row `CTMS-075` is the direct scope and acceptance source.
...
### Scenario: Detect Emergency Questions

1. Actor opens or triggers the `Detect Emergency Questions` workflow.
...
If dependency data from `CTMS-072, CTMS-077` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Detect Emergency Questions`:
...
- The approved PB V3.1 acceptance behavior for `Detect Emergency Questions` is implemented as explicit system behavior..
...
- The actor or system attempts `Detect Emergency Questions`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-078-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-078-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Detect Emergency Questions` if they are not already implemented?

Affected:
- Jira Story: `CTMS-075`
- Logic Subtask: `CTMS-075-T01`
- UI Subtask: `CTMS-075-T02`
...
- Jira Logic Subtask `CTMS-078-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-078-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Detect Emergency Questions`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-069, CTMS-074`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-78-detect-emergency-questions.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-78-detect-emergency-questions.md#ui-and-tests`
