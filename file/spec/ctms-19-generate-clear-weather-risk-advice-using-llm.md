...
**Spec Reference**  
/file/spec/ctms-19-generate-clear-weather-risk-advice-using-llm.md
...
Implement `Generate Clear Weather Risk Advice Using LLM` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Generate Clear Weather Risk Advice Using LLM`
- Story: As a System, I want to generate clear weather risk advice using llm so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Generate Clear Weather Risk Advice Using LLM` within `EPIC 3. Weather Risk Assessment`.
...
The system implements `Generate Clear Weather Risk Advice Using LLM` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Generate Clear Weather Risk Advice Using LLM` exactly as approved in the source backlog.
...
### Scenario: Generate Clear Weather Risk Advice Using LLM

1. Actor opens or triggers the `Generate Clear Weather Risk Advice Using LLM` workflow.
...
The implementation must persist or return only data required for `Generate Clear Weather Risk Advice Using LLM`:
...
- The approved PB V3.1 acceptance behavior for `Generate Clear Weather Risk Advice Using LLM` is implemented as explicit system behavior..
...
- The actor or system attempts `Generate Clear Weather Risk Advice Using LLM`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-019-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-019-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Generate Clear Weather Risk Advice Using LLM` if they are not already implemented?

Affected:
- Jira Story: `CTMS-019`
- Logic Subtask: `CTMS-019-T01`
- UI Subtask: `CTMS-019-T02`
...
- Jira Logic Subtask `CTMS-019-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-019-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Generate Clear Weather Risk Advice Using LLM`
- Priority: `Must Have`
- Story points: `8.0`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-19-generate-clear-weather-risk-advice-using-llm.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-19-generate-clear-weather-risk-advice-using-llm.md#ui-and-tests`
