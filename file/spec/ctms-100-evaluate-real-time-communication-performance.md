...
**Spec Reference**  
/file/spec/ctms-100-evaluate-real-time-communication-performance.md
...
Implement `Evaluate Real-Time Communication Performance` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Evaluate Real-Time Communication Performance`
- Story: As a System, I want to evaluate real-time communication performance so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Evaluate Real-Time Communication Performance` within `EPIC 17. Reports and Evaluation Metrics`.
...
- Dependencies are satisfied: CTMS-078, CTMS-079, CTMS-082.
...
The system implements `Evaluate Real-Time Communication Performance` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Evaluate Real-Time Communication Performance` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-281` | Required behavior for `Evaluate Real-Time Communication Performance` must validate and enforce SOS, P95, P99, Real, time, evaluation, release, gates, delivery, normal as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-265` | WebSocket latency measurement must store `sent_at`, `received_at`, `acknowledged_at`, `latency_ms`, and `event_type`, and support average/P95/P99 aggregation without storing sensitive payloads solely for measurement. |
...
- PB V3.1 row `CTMS-100` is the direct scope and acceptance source.
...
### Scenario: Evaluate Real-Time Communication Performance

1. Actor opens or triggers the `Evaluate Real-Time Communication Performance` workflow.
...
If dependency data from `CTMS-088` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Evaluate Real-Time Communication Performance`:
...
- The approved PB V3.1 acceptance behavior for `Evaluate Real-Time Communication Performance` is implemented as explicit system behavior..
...
- The actor or system attempts `Evaluate Real-Time Communication Performance`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-106-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-106-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Evaluate Real-Time Communication Performance` if they are not already implemented?

Affected:
- Jira Story: `CTMS-100`
- Logic Subtask: `CTMS-100-T01`
- UI Subtask: `CTMS-100-T02`
...
- Jira Logic Subtask `CTMS-106-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-106-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Evaluate Real-Time Communication Performance`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-078, CTMS-079, CTMS-082`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-049, BR-218, BR-252, BR-253, BR-255, BR-270, BR-271, BR-273, BR-274, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-106-evaluate-real-time-communication-performance.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-106-evaluate-real-time-communication-performance.md#ui-and-tests`
