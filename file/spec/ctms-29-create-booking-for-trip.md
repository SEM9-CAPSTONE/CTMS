...
Implement `Create Booking for Trip` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Create Booking for Trip`
- Story: As a System, I want to create booking for trip so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Create Booking for Trip` within `EPIC 5. Booking and Payment`.
...
- Dependencies are satisfied: CTMS-023, CTMS-024, CTMS-018.
...
The system implements `Create Booking for Trip` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Create Booking for Trip` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-082` | Required behavior for `Create Booking for Trip` must validate and enforce Booking, Trip, published, booking_deadline, approved, Route, version, eligible, Weather, Risk as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-084` | Required behavior for `Create Booking for Trip` must validate and enforce Trip, booking, pending_payment, payment_status, unpaid, hold_expires_at, confirmed, charge as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-048` | Required behavior for `Create Booking for Trip` must validate and enforce Trip, bookings, booking_members, entity, Member as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-215` | Required behavior for `Create Booking for Trip` must validate and enforce booking.trip_id, Camper, booking, trip_id, route_id as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |

### 5.5 Source Confidence

- PB V3.1 row `CTMS-029` is the direct scope and acceptance source.
...
### Scenario: Create Booking for Trip

1. Actor opens or triggers the `Create Booking for Trip` workflow.
...
If dependency data from `CTMS-023, CTMS-024, CTMS-018` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Create Booking for Trip`:
...
- The approved PB V3.1 acceptance behavior for `Create Booking for Trip` is implemented as explicit system behavior..
...
- The actor or system attempts `Create Booking for Trip`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-029-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-029-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Affected:
- Jira Story: `CTMS-029`
- Logic Subtask: `CTMS-029-T01`
- UI Subtask: `CTMS-029-T02`
...
- Jira Logic Subtask `CTMS-029-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-029-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Create Booking for Trip`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-023, CTMS-024, CTMS-018`
- Status: `To Do`
- Sprint: `Sprint 3`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-29-create-booking-for-trip.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-29-create-booking-for-trip.md#ui-and-tests`
