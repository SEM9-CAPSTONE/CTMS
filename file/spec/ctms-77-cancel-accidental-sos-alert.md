...
**Spec Reference**  
/file/spec/ctms-77-cancel-accidental-sos-alert.md
...
Implement `Cancel Accidental SOS Alert` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Cancel Accidental SOS Alert`
- Story: As a System, I want to cancel accidental sos alert so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Cancel Accidental SOS Alert` within `EPIC 12. SOS and Emergency Management`.
...
- Dependencies are satisfied: CTMS-074.
...
The system implements `Cancel Accidental SOS Alert` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Cancel Accidental SOS Alert` exactly as approved in the source backlog.
...
| ID | Content |
| --- | --- |
| `BR-258` | Required behavior for `Cancel Accidental SOS Alert` must validate and enforce SOS, cancel, configured, cancellation, policy, cancelled_by, cancelled_at, reason, history as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-077` is the direct scope and acceptance source.
...
### Scenario: Cancel Accidental SOS Alert

1. Actor opens or triggers the `Cancel Accidental SOS Alert` workflow.
...
If dependency data from `CTMS-077` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Cancel Accidental SOS Alert`:
...
- The approved PB V3.1 acceptance behavior for `Cancel Accidental SOS Alert` is implemented as explicit system behavior..
...
- The actor or system attempts `Cancel Accidental SOS Alert`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-081-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-081-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Cancel Accidental SOS Alert` if they are not already implemented?

Affected:
- Jira Story: `CTMS-077`
- Logic Subtask: `CTMS-077-T01`
- UI Subtask: `CTMS-077-T02`
...
- Jira Logic Subtask `CTMS-081-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-081-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Cancel Accidental SOS Alert`
- Priority: `Should Have`
- Story points: `3.0`
- Dependencies: `CTMS-074`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-81-cancel-accidental-sos-alert.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-81-cancel-accidental-sos-alert.md#ui-and-tests`
