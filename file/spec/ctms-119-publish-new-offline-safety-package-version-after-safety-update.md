...
**Spec Reference**  
/file/spec/ctms-119-publish-new-offline-safety-package-version-after-safety-update.md
...
Implement `Publish New Offline Safety Package Version after Safety Update` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Publish New Offline Safety Package Version after Safety Update`
- Story: As a System, I want to publish new offline safety package version after safety update so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Publish New Offline Safety Package Version after Safety Update` within `EPIC 8. Offline Package`.
...
- Dependencies are satisfied: CTMS-118, CTMS-010, CTMS-011, CTMS-014.
...
The system implements `Publish New Offline Safety Package Version after Safety Update` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Publish New Offline Safety Package Version after Safety Update` exactly as approved in the source backlog.
...
| `BR-428` | Required behavior for `Publish New Offline Safety Package Version after Safety Update` must validate and enforce Package, generation, snapshot, reference, versions, authoritative, inputs, version, chung, Route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-429` | Required behavior for `Publish New Offline Safety Package Version after Safety Update` must validate and enforce Trip, download, resolve, package, version, server, side, policy, client, revoked as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-430` | Required behavior for `Publish New Offline Safety Package Version after Safety Update` must validate and enforce Offline, Safety, Package, revoke, safety, revocation, metadata, Trip, resolve as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-119` is the direct scope and acceptance source.
...
### Scenario: Publish New Offline Safety Package Version after Safety Update

1. Actor opens or triggers the `Publish New Offline Safety Package Version after Safety Update` workflow.
...
If dependency data from `CTMS-124, CTMS-010, CTMS-011, CTMS-014` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Publish New Offline Safety Package Version after Safety Update`:
...
- The approved PB V3.1 acceptance behavior for `Publish New Offline Safety Package Version after Safety Update` is implemented as explicit system behavior..
...
- The actor or system attempts `Publish New Offline Safety Package Version after Safety Update`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-125-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-125-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Publish New Offline Safety Package Version after Safety Update` if they are not already implemented?

Affected:
- Jira Story: `CTMS-119`
- Logic Subtask: `CTMS-119-T01`
- UI Subtask: `CTMS-119-T02`
...
- Jira Logic Subtask `CTMS-125-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-125-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Publish New Offline Safety Package Version after Safety Update`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-118, CTMS-010, CTMS-011, CTMS-014`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Story-level business rules: BR-231, BR-233, BR-235, BR-237, BR-365, BR-371, BR-372, BR-437, BR-438
- Jira execution tasks should reference:
  - `/file/spec/ctms-125-publish-new-offline-safety-package-version-after-safety-update.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-125-publish-new-offline-safety-package-version-after-safety-update.md#ui-and-tests`
