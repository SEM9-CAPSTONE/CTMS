...
Implement `Record GPS Breadcrumbs during Trekking` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Record GPS Breadcrumbs during Trekking`
- Story: As a System, I want to record gps breadcrumbs during trekking so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Record GPS Breadcrumbs during Trekking` within `EPIC 9. GPS Navigation and Route Deviation`.
...
- Dependencies are satisfied: CTMS-058.
...
The system implements `Record GPS Breadcrumbs during Trekking` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Record GPS Breadcrumbs during Trekking` exactly as approved in the source backlog.
...
| `BR-367` | Required behavior for `Record GPS Breadcrumbs during Trekking` must validate and enforce GPS, Trip, client, active, package, context, safety, session, event, sinh as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-369` | Required behavior for `Record GPS Breadcrumbs during Trekking` must validate and enforce GPS, sample, timestamp, location, provider, accuracy, altitude, speed, bearing as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
| `BR-370` | Required behavior for `Record GPS Breadcrumbs during Trekking` must validate and enforce Sample, timestamp, location, latitude, longitude, malformed, safety, calculation, diagnostic, route as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-059` is the direct scope and acceptance source.
...
### Scenario: Record GPS Breadcrumbs during Trekking

1. Actor opens or triggers the `Record GPS Breadcrumbs during Trekking` workflow.
...
If dependency data from `CTMS-058` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Record GPS Breadcrumbs during Trekking`:
...
- The approved PB V3.1 acceptance behavior for `Record GPS Breadcrumbs during Trekking` is implemented as explicit system behavior..
...
- The actor or system attempts `Record GPS Breadcrumbs during Trekking`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-059-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-059-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Record GPS Breadcrumbs during Trekking` if they are not already implemented?

Affected:
- Jira Story: `CTMS-059`
- Logic Subtask: `CTMS-059-T01`
- UI Subtask: `CTMS-059-T02`
...
- Jira Logic Subtask `CTMS-059-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-059-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Record GPS Breadcrumbs during Trekking`
- Priority: `Must Have`
- Story points: `8.0`
- Dependencies: `CTMS-058`
- Status: `To Do`
- Sprint: `Sprint 4`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-59-record-gps-breadcrumbs-during-trekking.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-59-record-gps-breadcrumbs-during-trekking.md#ui-and-tests`
