...
**Spec Reference**  
/file/spec/ctms-86-manage-trip-incidents.md
...
Implement `Manage Trip Incidents` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Manage Trip Incidents`
- Story: As a System, I want to manage trip incidents so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Manage Trip Incidents` within `EPIC 14. Host Monitoring Dashboard`.
...
- Dependencies are satisfied: CTMS-049, CTMS-074.
...
The system implements `Manage Trip Incidents` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Manage Trip Incidents` exactly as approved in the source backlog.
...
### Scenario: Manage Trip Incidents

1. Actor opens or triggers the `Manage Trip Incidents` workflow.
...
If dependency data from `CTMS-049, CTMS-077` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Manage Trip Incidents`:
...
- The approved PB V3.1 acceptance behavior for `Manage Trip Incidents` is implemented as explicit system behavior..
...
- The actor or system attempts `Manage Trip Incidents`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-092-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-092-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Manage Trip Incidents` if they are not already implemented?

Affected:
- Jira Story: `CTMS-086`
- Logic Subtask: `CTMS-086-T01`
- UI Subtask: `CTMS-086-T02`
...
- Jira Logic Subtask `CTMS-092-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-092-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Manage Trip Incidents`
- Priority: `Should Have`
- Story points: `8.0`
- Dependencies: `CTMS-049, CTMS-074`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-92-manage-trip-incidents.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-92-manage-trip-incidents.md#ui-and-tests`
