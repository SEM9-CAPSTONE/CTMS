...
**Spec Reference**  
/file/spec/ctms-80-acknowledge-and-close-sos-handling.md
...
Implement `Acknowledge and Close SOS Handling` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Acknowledge and Close SOS Handling`
- Story: As a System, I want to acknowledge and close sos handling so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Acknowledge and Close SOS Handling` within `EPIC 12. SOS and Emergency Management`.
...
- Dependencies are satisfied: CTMS-079.
...
The system implements `Acknowledge and Close SOS Handling` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Acknowledge and Close SOS Handling` exactly as approved in the source backlog.
...
- PB V3.1 row `CTMS-080` is the direct scope and acceptance source.
...
### Scenario: Acknowledge and Close SOS Handling

1. Actor opens or triggers the `Acknowledge and Close SOS Handling` workflow.
...
If dependency data from `CTMS-083` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Acknowledge and Close SOS Handling`:
...
- The approved PB V3.1 acceptance behavior for `Acknowledge and Close SOS Handling` is implemented as explicit system behavior..
...
- The actor or system attempts `Acknowledge and Close SOS Handling`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-084-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-084-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Acknowledge and Close SOS Handling` if they are not already implemented?

Affected:
- Jira Story: `CTMS-080`
- Logic Subtask: `CTMS-080-T01`
- UI Subtask: `CTMS-080-T02`
...
- Jira Logic Subtask `CTMS-084-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-084-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Acknowledge and Close SOS Handling`
- Priority: `Must Have`
- Story points: `5.0`
- Dependencies: `CTMS-079`
- Status: `To Do`
- Sprint: `Sprint 5`
...
- Story-level business rules: BR-270, BR-271, BR-273, BR-274, BR-018, BR-222, BR-223, BR-224
- Jira execution tasks should reference:
  - `/file/spec/ctms-84-acknowledge-and-close-sos-handling.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-84-acknowledge-and-close-sos-handling.md#ui-and-tests`
