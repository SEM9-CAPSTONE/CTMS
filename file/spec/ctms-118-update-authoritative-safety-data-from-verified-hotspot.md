...
**Spec Reference**  
/file/spec/ctms-118-update-authoritative-safety-data-from-verified-hotspot.md
...
Implement `Update Authoritative Safety Data from Verified Hotspot` so the CTMS workflow is safe, consistent, auditable, and aligned with PB V3.1.

Business purpose from PB V3.1:

- English use case: `Update Authoritative Safety Data from Verified Hotspot`
- Story: As a System, I want to update authoritative safety data from verified hotspot so that CTMS supports the workflow safely and consistently.
...
### In Scope
- The behavior needed for `Update Authoritative Safety Data from Verified Hotspot` within `EPIC 2. Trekking Route and Checkpoint Management`.
...
- Dependencies are satisfied: CTMS-117, CTMS-103.
...
The system implements `Update Authoritative Safety Data from Verified Hotspot` exactly within the PB V3.1 scope:

- Implement the PB V3.1 acceptance behavior for `Update Authoritative Safety Data from Verified Hotspot` exactly as approved in the source backlog.
...
| `BR-419` | Required behavior for `Update Authoritative Safety Data from Verified Hotspot` must validate and enforce CONFIRMED, corrective, action, PotentialSafetyHotspot, hotspot, evidence, review, decision, authoritative as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
| `BR-426` | Required behavior for `Update Authoritative Safety Data from Verified Hotspot` must validate and enforce Update, authoritative, Route, geometry, checkpoint, hazard, validation, authorization, module, safety as part of the story-specific business contract. Backend checks must run before persistence, violations must be rejected without partial side effects, UI must show blocked or conflict states where relevant, and tests must cover both allowed and violation paths. |
...
- PB V3.1 row `CTMS-118` is the direct scope and acceptance source.
...
### Scenario: Update Authoritative Safety Data from Verified Hotspot

1. Actor opens or triggers the `Update Authoritative Safety Data from Verified Hotspot` workflow.
...
If dependency data from `CTMS-123, CTMS-109` is missing or not in an allowed state, block the workflow with a clear reason.
...
The implementation must persist or return only data required for `Update Authoritative Safety Data from Verified Hotspot`:
...
- The approved PB V3.1 acceptance behavior for `Update Authoritative Safety Data from Verified Hotspot` is implemented as explicit system behavior..
...
- The actor or system attempts `Update Authoritative Safety Data from Verified Hotspot`.
...
- Keep this as the HOW-SYSTEM responsibility contract for `CTMS-124-T01`; do not duplicate the complete end-to-end flow in Jira.
...
- Keep this as the HOW-CLIENT responsibility contract for `CTMS-124-T02`; backend/server responses remain the source of truth for server-owned business state.
...
Question:
What are the final endpoint paths, request DTOs, response DTOs, and error payloads for `Update Authoritative Safety Data from Verified Hotspot` if they are not already implemented?

Affected:
- Jira Story: `CTMS-118`
- Logic Subtask: `CTMS-118-T01`
- UI Subtask: `CTMS-118-T02`
...
- Jira Logic Subtask `CTMS-124-T01` owns HOW-SYSTEM responsibilities.
- Jira UI Subtask `CTMS-124-T02` owns HOW-CLIENT responsibilities when a client workflow exists.
- This file/spec owns the detailed execution flow, edge cases, contracts, invariants, and technical processing.
- Product Backlog V3.1 use case: `Update Authoritative Safety Data from Verified Hotspot`
- Priority: `Should Have`
- Story points: `13.0`
- Dependencies: `CTMS-117, CTMS-103`
- Status: `To Do`
- Sprint: `Sprint 5`
- Commitment: `Stretch`
...
- Jira execution tasks should reference:
  - `/file/spec/ctms-124-update-authoritative-safety-data-from-verified-hotspot.md#backend-preparation-logic-and-tests`
  - `/file/spec/ctms-124-update-authoritative-safety-data-from-verified-hotspot.md#ui-and-tests`
