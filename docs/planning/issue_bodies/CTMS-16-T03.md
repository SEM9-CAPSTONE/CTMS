## Parent Story
- External Story ID: `CTMS-16`
- Story title: Phê duyệt campsite trước khi công khai
- Epic: `ctms-epic-02`
- Sprint: `Sprint 2`

## Requirement
- Implement validation, boundary handling, conflict handling and recoverable error behavior.
- Acceptance Criteria: Chỉ campsite có status = pending_approval mới được xét duyệt; duyệt thành công chuyển status = active. Nếu không duyệt, chuyển về draft để Host chỉnh sửa và lưu lý do trong audit_logs/notifications; không sử dụng status rejected vì database không có giá trị này.
